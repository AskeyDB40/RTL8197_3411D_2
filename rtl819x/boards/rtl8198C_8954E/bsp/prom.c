...
#if 1 /* reserve memory page mapping to SRAM */
#define MEM_RESERVE_OFFSET 0x04000000
#if defined(CONFIG_PAGE_SIZE_4KB)
#define MEM_RESERVE_SIZE 0x00001000
#elif defined(CONFIG_PAGE_SIZE_16KB)
#define MEM_RESERVE_SIZE 0x00004000
#elif defined(CONFIG_PAGE_SIZE_64KB)
#define MEM_RESERVE_SIZE 0x00010000
#else
#error "wrong PAGE_SIZE!!"
#endif

void __init make_memory_reserved(u_long mem_size)
{
	u_long reserve_offset = MEM_RESERVE_OFFSET;

	if (mem_size > MEM_RESERVE_OFFSET) {
		add_memory_region(0, 0x04000000, BOOT_MEM_RAM);
		if (mem_size-MEM_RESERVE_OFFSET > MEM_RESERVE_SIZE) {
			add_memory_region(MEM_RESERVE_OFFSET, MEM_RESERVE_SIZE, BOOT_MEM_RESERVED);
			reserve_offset += MEM_RESERVE_SIZE;
			add_memory_region(reserve_offset, mem_size-reserve_offset, BOOT_MEM_RAM);
		}
		else {
			add_memory_region(MEM_RESERVE_OFFSET, mem_size-MEM_RESERVE_OFFSET, BOOT_MEM_RESERVED);
		}
	}
	else {
...
#endif /* reserve memory page mapping to SRAM */

inline void __init disable_SRAM_mapping(void)
{
	REG32(0xb8004000) &= ~(0x00000001);
	REG32(0xb8001300) &= ~(0x00000001);
}
...
#if 1 /* reserve memory page mapping to SRAM */
	if ((REG32(0xb8004000) & 0x00000001) && (REG32(0xb8001300) & 0x00000001)) {
		reserve_mem = 1;
	}
	//printk("==> %s:%d, reserve_mem=%d\n", __func__, __LINE__, reserve_mem);//bruce
#else
	/* disable SRAM MAPPING */
	disable_SRAM_mapping();
#endif
