...
#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT
#ifdef CONFIG_ISP_IGMPPROXY_MULTIWAN
	printf("addsmux protocol real_dev smux_dev [napt] [brpppoe] [vlan vid] [igmpproxy enable onphy]\n\tprotocol: bridge/ipoe/pppoe\n\treal_dev: smux device name\n\tsmux_dev: real dev name\n");
#else
...
int main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "addsmux") || !strcmp(argv[1], "remsmux"))
	{
		...
		while(idx < argc)
		{
			...
			#ifdef CONFIG_ISP_IGMPPROXY_MULTIWAN
			else if((!strcmp(argv[idx], "igmpproxy")) && (idx+2<argc))
			{	
				sifr.igmpproxy_enable = atoi(argv[idx+1])?1:0;
				sifr.on_phy = atoi(argv[idx+2])?1:0;
				idx += 2;
			}
			#endif
			idx++;
	...
	if (!strcmp(argv[1], "port_mapping"))
	{
		...
		if (ioctl(fd, SIOCSITFGROUP, &ifr) < 0) {
			printf("Error set port mapping in ethctl\n");
			close(fd);
			#if defined(RTL_FILE_LOCK_SUPPORT)
			ethctl_unlock(lock_fd, lock);
			#endif
