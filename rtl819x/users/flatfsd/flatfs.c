...
int flat_check(void)
{
    ...
    if((rc = flat_get_mtd(mtdname)) < 0)
        return rc;

	if (chdir(DSTDIR) < 0)
		return ERROR_CODE();

	if ((rc = flat_open(mtdname, "r")) < 0)
		return rc;
...
/*****************************************************************************/
/*
 * Get mtd partition for flatfs
 */
int flat_get_mtd(char *mtdname)
{
    if(mtdname==NULL)
        return -1; 

#ifdef MTD_NAME_MAPPING
    rtl_name_to_mtdchar(FLATFSD_MTD_NAME, mtdname);
#else
    sprintf(mtdname, "/dev/mtd4");
#endif

    printf("[%s:%d]use mtd partition:%s\n", __FUNCTION__, __LINE__, mtdname);
    return 0;
}
