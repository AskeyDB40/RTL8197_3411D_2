...
#ifdef DEBUG
	syslog(LOG_DEBUG, "flat_checkfs() calculated checksum over %d bytes = %u", len, sum);
#endif
	...
	*total = sizeof(hdr);
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		if ((strcmp(dp->d_name, ".") == 0) ||
		    (strcmp(dp->d_name, "..") == 0) ||
		    (strcmp(dp->d_name, "DefaultCwmpNotify.txt") == 0) ||
		    (strcmp(dp->d_name, FLATFSD_CONFIG) == 0))
			continue;

		rc = writefile(dp->d_name, total, dowrite);
		if (rc < 0) {
			//syslog(LOG_ERR, "Failed to write write file %s (%d): %d",dp->d_name, rc, errno);
			if (!ret)
				ret = rc;
		}
	}
