...
static void save_config_to_flash(void)
{
	if (!nowrite) {
		...
		if ((rc = flat_savefs(fsver)) < 0)
			printf("Failed to write flatfs (%d)\n", rc);
#endif
#ifdef LOGGING
...
static void reset_config_fs(void)
{
	...
	if ((rc = flat_clean(0)) < 0) {
		syslog(LOG_ERR, "Failed to prepare flatfs for reset (%d)", rc);
		exit(1);
	}
...
int creatpidfile(void)
{
	...
	if ((f = fopen(pidfile, "w")) == NULL) {
		//syslog(LOG_ERR, "Failed to open %s",pidfile);
		return -1;
	}
	fprintf(f, "%d\n", pid);
	fclose(f);
...
int register_resetpid(void)
{
	...
	if ((fd = open("/dev/ledman", O_RDONLY)) < 0) {
		syslog(LOG_ERR, "Failed to open /dev/ledman");
		return -1;
	}
	if (ioctl(fd, LEDMAN_CMD_SIGNAL, 0) < 0) {
		syslog(LOG_ERR, "Failed to register pid");
		return -2;
	}
	close(fd);
#endif
...
int main(int argc, char *argv[])
{
	...
	while ((rc = getopt(argc, argv, "vcnribw123hs?")) != EOF) {
		switch (rc) {
		case 'w':
			clobbercfg++;
			readonly++;
	...
	if (readonly) {
		...
		if (clobbercfg ||
			...
			fprintf(stderr, "filecount=%d, needinit=%d\n", rc1, rc2);
			//syslog(LOG_ERR, "filecount=%d, needinit=%d", rc1, rc2);
#ifdef LOGGING
			...
			//fprintf(stderr, "Nonexistent or bad flatfs (%d), creating new one...\n", rc);
			//flat_clean(1);
			//if ((rc = flat_new(DEFAULTDIR)) < 0) {
			//	syslog(LOG_ERR, "Failed to create new flatfs, err=%d errno=%d",
			//		rc, errno);
			//		fprintf(stderr, "Failed to create new flatfs, err=%d errno=%d\n", rc, errno);
			//	exit(1);
			//}
			//save_config_to_flash();
		}
	...
	for (;;) {
		if (recv_usr1) {
			...
			if (stat(IGNORE_FLASH_WRITE_FILE, &st_buf) == 0) {
				syslog(LOG_INFO, "Not writing to flash "
					"because %s exists",
					IGNORE_FLASH_WRITE_FILE);
				fprintf(stderr, "Not writing to flash because %s exists\n",IGNORE_FLASH_WRITE_FILE);
				continue;
			}			
			save_config_to_flash();			
			continue;
		...
		if (current_cmd)
			led_pause();
		else if (!recv_hup && !recv_usr1 && !recv_usr2)
			pause();
	}
