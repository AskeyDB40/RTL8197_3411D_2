...
char option43[]={43, 6, 1, 4, CONFIGVERSTR0, CONFIGVERSTR1, CONFIGVERSTR2, CONFIGVERSTR3,0,0};
#endif

#ifdef CTC_DHCP_OPTION60
char option60[]={0x3c, 0x1F, 0x12 , 0x34, 0x01, 0x07, 'R', 'e', 'a', 'l', 't', 'e', 'k', 0x02, 0x03, 'C', 'P', 'E', 0x03, 0x08, 'E', '8', 'R', 'O', 'U', 'T', 'E', 'R', 0x04, 0x03, '1', '.', '0', 0, 0}; // FOR dhcp ack, we just send enterprise code to clients.
#endif
...
void initDhcpdOption(unsigned char type, unsigned char len, char *value)
{
	int entrynum=0, i, found=0;
	...
	{
		*((char *)&entry) = (char)i;
		if(!apmib_get(MIB_DHCP_SERVER_OPTION_TBL, (void *)&entry))
			continue;		

		if(entry.tag==type && entry.usedFor==eUsedFor_DHCPServer)
		{
			found=1;
			break;	
		}
	}
	if(found==0 && entrynum<MAX_DHCP_SERVER_OPTION_NUM)
	{
		entry.enable=1;
		entry.usedFor=eUsedFor_DHCPServer;
		entry.order=entrynum+1;
		entry.tag=type;
		entry.len=len;
		memcpy(entry.value, value, entry.len);
		entry.ifIndex=server_config.ifindex;

		apmib_set(MIB_DHCP_SERVER_OPTION_DEL, (void *)&entry);
	
		if (!apmib_set(MIB_DHCP_SERVER_OPTION_ADD, (void *)&entry))
		{		
			printf("%s:%d apmib_set MIB_DHCP_SERVER_OPTION_ADD fails!####\n",__FUNCTION__,__LINE__);
			return;		
		}
		else
		{
...
int addDhcpdOption(struct dhcpMessage *packet)
{
	MIB_CE_DHCP_OPTION_T entry;
	int i, entrynum;
	...
	for (i=1; i<=entrynum; i++) 
	{
		*((char *)&entry) = (char)i;
		...
		if ((server_config.poolorder==0XFFFFFFFF && entry.usedFor != eUsedFor_DHCPServer) ||
			(server_config.poolorder!=0XFFFFFFFF && entry.usedFor != eUsedFor_DHCPServer_ServingPool))
		{
			//printf("\n%s:%d entry.tag=%d continue!!!\n",__FUNCTION__,__LINE__,entry.tag);
			continue;
		}
...
#ifdef TR069_ANNEX_F
char option125[256]={0};
void generateDefaultOption125()
{
	char tmp[16];
	unsigned char optionStr[256] = {0};
	...
	//if (apmib_init())
	{
		...
		memcpy(option125, optionStr, option125Len+2);
	}	
}
...
void find_match_serving_pool(struct dhcpMessage *dhcppacket)
{
	...
	struct server_config_t* p_default_servingpool=p_servingpool_tmp;
	...
	while(p_servingpool_tmp!=NULL){
		...
		p_default_servingpool=p_servingpool_tmp;
	...
	if(p_servingpool_tmp==NULL) {		
		...
		//default pool is placed at the end of the pool list
		p_servingpool_tmp=p_default_servingpool;
		serverpool = 1;
	}
