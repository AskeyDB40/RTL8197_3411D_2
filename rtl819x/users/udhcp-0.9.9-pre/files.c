...
static int read_reserved_ip(char *const_line, void *arg)
{
	char *line=NULL, *token=NULL, *savestr=NULL;
	struct in_addr reserved_ip;
	line = (char *) const_line;
	//printf("\n%s line=%s\n",__FUNCTION__,line);
	int i=0;
	unsigned int *p_reserved_ip=(unsigned int *)arg;
	
	token=strtok_r(line," ",&savestr);
	while(token != NULL)
	{
		if (inet_aton(token, &reserved_ip)) 		
		{
			p_reserved_ip[i++]=reserved_ip.s_addr;
			//printf("\n%s p_reserved_ip[%d]=%08x\n",__FUNCTION__,i,p_reserved_ip[i]);
		}
...
#endif
#ifdef SUPPORT_RESERVED_DHCP_IPADDR
	{"reserved_ip",read_reserved_ip, (server_config.reservedDhcpIp),	""},
#endif
...
void read_leases(char *file)
{
	...
	//	printf("%s:%d read lease.yiaddr=%08x\n",__FUNCTION__,__LINE__,lease.yiaddr);
		...
		for(j=0;j<16;j++)
		{
			if(lease.yiaddr==server_config.reservedDhcpIp[j])
				break;
		}
		if(j<16)
		{
