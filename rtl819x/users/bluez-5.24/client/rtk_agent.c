...
static void rl_printf(const char *fmt, ...)
{
	va_list args;
...
static void register_agent_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		...
		agent_default(conn, agent_manager);
	} else {
...
static void unregister_agent_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == FALSE) {
		rl_printf("<unregister_agent_reply> rtk Agent unregistered\n");
		agent_release(conn);
	} else {
...
void agent_unregister(DBusConnection *conn, GDBusProxy *manager)
{
	...
	if (!manager) {
		rl_printf("<agent_unregister>rtk Agent unregistered\n");
		agent_release(conn);
		return;
	}
...
//static const char *uuidstr_to_str(const char *uuid)
//{
//	uint32_t val;
//
//	if (!uuid)
...
//		return NULL;
//
//	return (const  char *)uuid32_to_str(val);
//}
...
gboolean g_dbus_client_method_call(DBusConnection* dbusConn,
		const char *obj_path, const char *interface,
		const char *method, const char *addr,
		void (*method_call_cb)(DBusPendingCall *call, void *user_data),
		void *user_data)
{
	DBusMessage *msg;
	...
	msg = dbus_message_new_method_call("org.bluez", obj_path, interface,
			method);
	if (!msg) {
	...
	dbus_message_append_args(msg, DBUS_TYPE_STRING, &addr,
						DBUS_TYPE_INVALID);

	if (!g_dbus_send_message_with_reply(dbusConn, msg,
					&call, 300 * 1000)) {
	...
	dbus_pending_call_set_notify(call, method_call_cb, NULL, NULL);
...
static void cmd_ota_start_reply(DBusPendingCall *call, void *user_data)
{
	DBusError error;
	DBusMessage *message;
	DBusError err;
	...
	if (dbus_set_error_from_message(&error, message)) {
		rl_printf ("Failed to update remote: %s %s\n",
				error.name, error.message);
		dbus_error_free(&error);
		return;
	}

	rl_printf ("Start to update remote\n");
}
...
static void cmd_ota_start(const char *arg)
{
	...
	g_dbus_client_method_call(dbus_conn,
			"/org/bluez/hci0",
			"org.bluez.RcuOtaManager1",
			"OTAStart", arg,
			cmd_ota_start_reply, NULL);
...
static const struct {
	...
	{ "exit",         NULL,       cmd_quit },
	{ "help" },
	{ "ota-start", "<dev>", cmd_ota_start, "OTA Start", dev_generator },
	{ }
};

#define SYS_MAXARGS	32
#define CMD_MAX_LEN	128
int parse_line(char *line, char *argv[])
{
	int nargs = 0;
	/* There may be some \n in line, we should not print it directly. */
...
static void process_cmdline(gchar *input, gsize len)
{
	char *cmd, *arg;
	int i;
