...
static void set_adv_parameters(int fd)
{
	...
	//cmd.min_interval = cpu_to_le16(0x00a0);
	//cmd.max_interval = cpu_to_le16(0x00a0);
	cmd.min_interval = cpu_to_le16(0x0800);
	cmd.max_interval = cpu_to_le16(0x0800);
	//cmd.advtype = 0x03;		/* Non-connectable advertising */
	cmd.advtype = 0x00;		/* connectable undirected advertising */
	cmd.own_bdaddr_type = 0x01;	/* Use random address */
...
static void set_adv_data(int fd)
{
	...
	cmd.data[2] = 0x01; /* LE Limited Discoverable Mode */
	cmd.data[2] |= 0x04; /* BR/EDR Not Supported */

	cmd.data[3] = 0x03;
	cmd.data[4] = 0x03; /* complete list of 15-bit service class uuids */
	cmd.data[5] = 0x12;
	cmd.data[6] = 0x18;

	cmd.data[7] = 0x03;
	cmd.data[8] = 0x19; /* appearance */
	cmd.data[9] = 0xc1;
	cmd.data[10] = 0x03;

	cmd.data[11] = 0x04;
	cmd.data[12] = 0x09; /* complete local name */
	cmd.data[13] = 0x52;
	cmd.data[14] = 0x43;
	cmd.data[15] = 0x55;
	...
	cmd.length = 1 + cmd.data[0] + 1 + cmd.data[3] + 1 + cmd.data[7] +
		  1 + cmd.data[11] + 1 + cmd.data[16];
	...
	memset(cmd.data, 0, sizeof(cmd.data));
	cmd.data[0] = 0x03;
	cmd.data[1] = 0x19; /* appearance */
	cmd.data[2] = 0xc1;
	cmd.data[3] = 0x03;
	cmd.length = 1 + cmd.data[0];
	if (hci_send_cmd(fd, OGF_LE_CTL, OCF_LE_SET_SCAN_RESPONSE_DATA,
			 sizeof(cmd), &cmd))
		error("Send cmd for set rsp data error");
}

static void start_advertising(struct btd_adapter *adapter)
{
	uint8_t enable;
	int hci_fd;
	...
	hci_fd = hci_open_dev(adapter->dev_id);
	if (hci_fd < 0) {
