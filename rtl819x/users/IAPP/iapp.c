...
/*check new STA associated interface which is in iapp enable list */
int check_is_STA_from_iapp_enable_interface(u8 *sta)
{
	int  i=0,j,found=0;
	...
	if ( buff == 0 ) {
		HOSTAPD_DEBUG(HOSTAPD_DEBUG_MSGDUMPS,"Allocate buffer failed!\n");
		return 0;
	}
	while(strlen(hapd->wlan_iface[i]) != 0){
		if ( getWlStaInfo(hapd->wlan_iface[i],  (WLAN_STA_INFO_Tp)buff ) < 0 ) {
			HOSTAPD_DEBUG(HOSTAPD_DEBUG_MSGDUMPS,"Read wlan sta info failed!\n");
			i++;
			continue;
		}
			...
			if (FD_ISSET(hapd->readfifo, &netFD))
			{
				...
				if (nRead > 0)
				{
					...
					if (msg_type == DOT11_EVENT_ASSOCIATION_IND) {
						...
						check_lan_ip_state();
						if(check_is_STA_from_iapp_enable_interface(msg->MACAddr))
