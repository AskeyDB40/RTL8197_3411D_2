...
//    control_xmit (buf);
	...
	memset(l2tp_cmdBuf,0, sizeof(l2tp_cmdBuf));

	l2tp_ns = t->control_seq_num;

	sprintf(l2tp_cmdBuf, "flash set L2TP_NS %d", l2tp_ns);
	system(l2tp_cmdBuf);
#endif
...
log(LOG_DEBUG,"%s(%d):control->msgType(%d)\n",__FUNCTION__,__LINE__,c->msgtype);
    switch (c->msgtype)
    {
    case 0:
        /*
...
//        control_xmit (buf);
...
#if defined(RTL_L2TP_POWEROFF_PATCH)  //Patch for l2tp re-connet to server(winxp) after power off/on by jiawenjian
	{
		...
		unsigned char tmp_int[256];
		unsigned char *l2tp_cmdBuf = (unsigned char *)&tmp_int[0];

		memset(l2tp_cmdBuf,0, sizeof(l2tp_cmdBuf));

		 //To generate CDN to store.
		log (LOG_NOTICE,"%s(%d):To generate CDN to store.	+++++ ",__FUNCTION__,__LINE__);//Added for test
		buf = new_outgoing (t);
		add_message_type_avp (buf, CDN);
		add_result_code_avp (buf, RESULT_CLEAR, 0, myMsg, strlen(myMsg));

		log (LOG_NOTICE,"%s(%d):c->ourcid(%d),c->cid(%d),c->qcid(%d),c->serno(%d).  +++++ ",
		__FUNCTION__,__LINE__,c->ourcid,c->cid,c->qcid,c->serno);//Added for test
#ifdef TEST_HIDDEN
		add_callid_avp (buf, c->ourcid, t);
#else
		add_callid_avp (buf, c->ourcid);
		...
		sprintf(l2tp_cmdBuf,"flash set L2TP_PAYLOAD_LENGTH %d", buf->len);
		system(l2tp_cmdBuf);

		l2tp_payload = (unsigned char *) buf->start;
		memcpy((void *)&l2tp_ns_icrp, (void *)&l2tp_payload[8], 2);
		sprintf(l2tp_cmdBuf, "flash set L2TP_NS %d", l2tp_ns_icrp);
		system(l2tp_cmdBuf);

		//Try to store CDN into flash and then toss it here.
		sprintf(l2tp_cmdBuf, "flash set L2TP_PAYLOAD %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
		l2tp_payload[0], l2tp_payload[1], l2tp_payload[2], l2tp_payload[3], l2tp_payload[4], l2tp_payload[5], l2tp_payload[6], l2tp_payload[7],
		l2tp_payload[8], l2tp_payload[9], l2tp_payload[10], l2tp_payload[11], l2tp_payload[12], l2tp_payload[13], l2tp_payload[14], l2tp_payload[15],
		l2tp_payload[16], l2tp_payload[17], l2tp_payload[18], l2tp_payload[19], l2tp_payload[20], l2tp_payload[21], l2tp_payload[22], l2tp_payload[23],
		l2tp_payload[24], l2tp_payload[25], l2tp_payload[26], l2tp_payload[27], l2tp_payload[28], l2tp_payload[29], l2tp_payload[30], l2tp_payload[31],
		l2tp_payload[32], l2tp_payload[33], l2tp_payload[34], l2tp_payload[35], l2tp_payload[36], l2tp_payload[37], l2tp_payload[38], l2tp_payload[39],
		l2tp_payload[40], l2tp_payload[41], l2tp_payload[42], l2tp_payload[43], l2tp_payload[44], l2tp_payload[45], l2tp_payload[46], l2tp_payload[47],
		l2tp_payload[48], l2tp_payload[49], l2tp_payload[50], l2tp_payload[51], l2tp_payload[52], l2tp_payload[53]);
