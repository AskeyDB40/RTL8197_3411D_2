...
static void little_to_big_endian(yaffs_PackedTags2* data,int size)
{
	yaffs_PackedTags2 tmp;

	tmp.t.sequenceNumber = ((data->t.sequenceNumber & 0xff000000) >> 24) | ((data->t.sequenceNumber & 0x00ff0000) >> 8)
							| ((data->t.sequenceNumber & 0x0000ff00) << 8) | ((data->t.sequenceNumber & 0x000000ff) << 24);

	
	tmp.t.objectId = ((data->t.objectId & 0xff000000) >> 24) | ((data->t.objectId & 0x00ff0000) >> 8)
							| ((data->t.objectId & 0x0000ff00) << 8) | ((data->t.objectId & 0x000000ff) << 24);

	tmp.t.chunkId = ((data->t.chunkId & 0xff000000) >> 24) | ((data->t.chunkId & 0x00ff0000) >> 8)
							| ((data->t.chunkId & 0x0000ff00) << 8) | ((data->t.chunkId & 0x000000ff) << 24);

	tmp.t.byteCount = ((data->t.byteCount & 0xff000000) >> 24) | ((data->t.byteCount & 0x00ff0000) >> 8)
							| ((data->t.byteCount & 0x0000ff00) << 8) | ((data->t.byteCount & 0x000000ff) << 24);


	memcpy(data,&tmp,sizeof(yaffs_PackedTags2));
}
...
// added NCB **CHECK**
	...
	if(convert_endian)
		little_to_big_endian(&pt,sizeof(yaffs_PackedTags2));
