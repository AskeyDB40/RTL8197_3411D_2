...
int main(int argc, char** argv)
{
	...
	if(argc == 2 && !strcmp(argv[1], "mib_flash_size_chk")){
		...
		if(sizeof(RF_DPK_SETTING_T)+sizeof(PARAM_HEADER_T)>=RF_DPK_SETTING_SECTOR_LEN){		
			printf("==============================================\n");
			printf("RF hardware setting over size!!\nFlash left %x but need %x\n",RF_DPK_SETTING_SECTOR_LEN,sizeof(RF_DPK_SETTING_T)+sizeof(PARAM_HEADER_T));
			exit(1);
		}
