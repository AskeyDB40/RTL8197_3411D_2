...
#ifdef CUSTOMER_HW_SETTING_SUPPORT
					if(CUSTOMER_HW_SETTING_OFFSET%erasesize){
...
//				 fprintf(stderr,"%s:%d header.len=0x%x\n",__FUNCTION__,__LINE__,header.len);
	...
	if(isAll==DEFAULT_ALL||isAll==DEFAULT_RF_DPK)
	{
		...
		sprintf((char *)header.signature, "%s%02d", RF_DPK_SETTING_HEADER_TAG, RF_DPK_SETTING_VER);
		header.len = sizeof(rfHwMib) + sizeof(checksum);
