...
char *apmib_rfDPKconf(void)
{
	...
	rfDpkHeader.len=HEADER_SWAP(rfDpkHeader.len);
    ...
    if (ok < 0) {
        rfDpkHeader.len = sizeof(RF_DPK_SETTING_T) + sizeof(unsigned char);
    }
    else {
        length = rfDpkHeader.len;
    }
    ...
    if (ok < 0) {
        ...
        DPK_DEBUG("Copy DPK zero MIB!!\n");
        if (ok == -1 || ok == -3)
            printf("Invalid RF DPK setting signature [sig=%c%c]!\n", rfDpkHeader.signature[0],rfDpkHeader.signature[1]);
        if (ok == -2 || ok == -3)
            printf("Invalid RF DPK setting version number or data length[ver=%d, len=%d size=%d]!\n", ver, length,sizeof(RF_DPK_SETTING_T));

    	memset(&rfDpkHeader, 0, sizeof(rfDpkHeader));
    	memset(&rfHwMib, 0, sizeof(rfHwMib));
    	data = NULL;
    	checksum = 0;

    	/* write RF DPK Setting */
    	sprintf((char *)rfDpkHeader.signature, "%s%02d", RF_DPK_SETTING_HEADER_TAG, RF_DPK_SETTING_VER);
    	rfDpkHeader.len = sizeof(rfHwMib) + sizeof(checksum);
    	...
    	data = (char *)&rfHwMib;			 
    	checksum = CHECKSUM((unsigned char *)data, rfDpkHeader.len-1);

        memcpy(buff, (char *)&rfHwMib, sizeof(rfHwMib));
        memcpy(buff + sizeof(rfHwMib), (char *)&checksum, sizeof(checksum));
    } 
    else {
...
//mib_display_tlv_content(RF_DPK_SETTING, data, rfDpkHeader.len);
        ...
        if ( memcmp(rfDpkHeader.signature, RF_DPK_SETTING_HEADER_TAG, TAG_LEN)) { /* invalid signature */
            ...
            write_header = 1;
            sprintf((char *)rfDpkHeader.signature, "%s%02d", RF_DPK_SETTING_HEADER_TAG, RF_DPK_SETTING_VER);
            rfDpkHeader.len=HEADER_SWAP(len);
        }
