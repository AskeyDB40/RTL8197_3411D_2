...
static inline int rtk_vid_based_priority_assin(int _num, char* _param[], struct qos_cmd_info_s* send_data)
{
	...
	if(_num > 13)		/*we define input 5 vids tops: 3+2*5*/
		goto QOS_ERROR;
	...
	if((_param[3] != NULL) && (!(memcmp(_param[3],"Vid_Based",strlen(_param[3]))))){
		...
		{
			temp = _param[i][3];
			if((_param[i] != NULL) && (strlen(_param[i]) == 4) && (!(memcmp(_param[i], "vid", (strlen(_param[i]) -1))))){				
				if((_param[i+1] != NULL) && (strlen(_param[i+1]) == 1) && (*(_param[i+1]) >= '0') && (*(_param[i+1]) < '8')){
				send_data->qos_data.vid_based_priority.vid[i-4] = temp;
				send_data->qos_data.vid_based_priority.pri[i-4] = *(_param[i+1]);	
				
				ret += 1;
				RTK_QOS_PRINTF("Priority %c is good for vid[%d]\n", send_data->qos_data.vid_based_priority.pri[i], send_data->qos_data.vid_based_priority.vid[i]);
				}else{
		 			goto QOS_ERROR;
				}
			}
