...
void send_peer_virtual_ip_to_kernel(uint8_t *real_ip_port, uint32_t virtual_ip)
{	
	int offset=0;
	...
	nl_send_buf[offset]=PEER_IP_FLAG;
	nl_send_buf[offset+1]=sizeof(uint32_t);
	memcpy(nl_send_buf+offset+2, real_ip_port, sizeof(uint32_t));
	offset+=2+sizeof(uint32_t);

	nl_send_buf[offset]=PEER_PORT_FLAG;
	nl_send_buf[offset+1]=sizeof(uint16_t);
	memcpy(nl_send_buf+offset+2, real_ip_port+sizeof(uint32_t), sizeof(uint16_t));
	offset+=2+sizeof(uint16_t);		

	nl_send_buf[offset]=VIRTUAL_IP_FLAG;
	nl_send_buf[offset+1]=sizeof(uint32_t);
	memcpy(nl_send_buf+offset+2, &virtual_ip, sizeof(uint32_t));
	offset+=2+sizeof(uint32_t);	

	send_netlink_msg(nl_send_buf, offset, VIRTUAL_IP_FLAG);	
  ...
  if (tls_authentication_status (mi->context.c2.tls_multi, 0) == TLS_AUTHENTICATION_SUCCEEDED)
      ...
      if (cc_succeeded)
	{
	  ...
	  if (TUNNEL_TYPE (mi->context.c1.tuntap) == DEV_TYPE_TUN)
		...
		#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
		send_peer_virtual_ip_to_kernel(mi->real.addr, mi->context.c2.push_ifconfig_local);	
		#endif		  
