...
extern int get_peer_ip_port(uint8_t *out, int *offset);

void send_crypto_key_to_kernel(uint8_t *enc_key, uint8_t enc_key_len, 
							uint8_t *enc_hmac_key, uint8_t enc_hmac_key_len,
							uint8_t *dec_key, uint8_t dec_key_len, 
							uint8_t *dec_hmac_key, uint8_t dec_hmac_key_len
							)
{
	int key_len;
	int offset=0;
	...
	get_peer_ip_port(nl_send_buf, &offset);
	
	nl_send_buf[offset]=ENC_KEY_FLAG;
	nl_send_buf[offset+1]=enc_key_len;
	memcpy(nl_send_buf+offset+2, enc_key, enc_key_len);
	offset+=(2+enc_key_len);

	nl_send_buf[offset]=HMAC_ENC_KEY_FLAG;
	nl_send_buf[offset+1]=enc_hmac_key_len;
	memcpy(nl_send_buf+offset+2, enc_hmac_key, enc_hmac_key_len);
	offset+=(2+enc_hmac_key_len);


	nl_send_buf[offset]=DEC_KEY_FLAG;
	nl_send_buf[offset+1]=dec_key_len;
	memcpy(nl_send_buf+offset+2, dec_key, dec_key_len);
	offset+=(2+dec_key_len);

	nl_send_buf[offset]=HMAC_DEC_KEY_FLAG;
	nl_send_buf[offset+1]=dec_hmac_key_len;
	memcpy(nl_send_buf+offset+2, dec_hmac_key, dec_hmac_key_len);
	offset+=(2+dec_hmac_key_len);
	
	send_netlink_msg(nl_send_buf, offset, CRYPT_KEY_FLAG);
			...
			bool server)
{
 ...
 struct key *enc_key=&key2.keys[(int)server];
 struct key *dec_key=&key2.keys[1-(int)server];

 send_crypto_key_to_kernel(enc_key->cipher, key_type->cipher_length, 
							  enc_key->hmac, key_type->hmac_length,
							  dec_key->cipher, key_type->cipher_length, 
							  dec_key->hmac, key_type->hmac_length
							  );
#endif
