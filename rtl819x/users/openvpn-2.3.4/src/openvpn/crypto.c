...
static struct sockaddr_nl netlink_src_addr, netlink_dest_addr;
struct nlmsghdr *netlink_nlh;
static struct iovec netlink_iov;
int netlink_sock;
static struct msghdr netlink_msg;

//uint8_t ip_port_string[256];

#define OPENVPN_NETLINK_CRYPTO 27
#define MAX_OPENVPN_PAYLOAD 1600

#define DIGEST_AND_IV_OFFSET	36	//20+16
#define DIGEST_AND_IV_OFFSET	36	//20+16
#define CBC_BLOCK_SIZE			16

//enum {DEC_KEY_FLAG=0, ENC_KEY_FLAG=1, HMAC_DEC_KEY_FLAG=2,HMAC_ENC_KEY_FLAG=4, DATA_TO_ENC_FLAG=8, DATA_TO_DEC_FLAG=16};
...
#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
int create_netlink_sock()
{
	netlink_sock=socket(PF_NETLINK, SOCK_RAW, OPENVPN_NETLINK_CRYPTO);
	if(netlink_sock<0)
	{
	...
	netlink_src_addr.nl_pid = getpid(); /* self pid */
	netlink_src_addr.nl_groups = 0; /* not in mcast groups */
	bind(netlink_sock, (struct sockaddr *)&netlink_src_addr, sizeof(netlink_src_addr));

	memset(&netlink_dest_addr, 0, sizeof(netlink_dest_addr));
	netlink_dest_addr.nl_family = AF_NETLINK;
	netlink_dest_addr.nl_pid = 0; /* For Linux Kernel */
	netlink_dest_addr.nl_groups = 0; /* unicast */

	netlink_nlh=(struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_OPENVPN_PAYLOAD));	
	if(netlink_nlh==NULL)
		return -1;
...
int send_netlink_msg(unsigned char *data, unsigned int len, unsigned short flag)
{
	/* Fill the netlink message header */
	memset(netlink_nlh, 0, NLMSG_SPACE(MAX_OPENVPN_PAYLOAD));
	
	netlink_nlh->nlmsg_len = NLMSG_HDRLEN+len;
	netlink_nlh->nlmsg_pid = getpid(); /* self pid */
	netlink_nlh->nlmsg_flags = flag;

	memcpy(NLMSG_DATA(netlink_nlh), data, len);

	memset(&netlink_msg, 0, sizeof(netlink_msg));
	memset(&netlink_iov, 0, sizeof(netlink_iov));
	
	netlink_iov.iov_base = (void *)netlink_nlh;
	netlink_iov.iov_len = netlink_nlh->nlmsg_len;
	netlink_msg.msg_name = (void *)&netlink_dest_addr;
	netlink_msg.msg_namelen = sizeof(netlink_dest_addr);
	netlink_msg.msg_iov = &netlink_iov;
	netlink_msg.msg_iovlen = 1;
	
	sendmsg(netlink_sock, &netlink_msg, 0);

	return 0;
}

int recv_netlink_msg(unsigned char *dst_buff, unsigned int *len, int flags)
{
	/* Fill the netlink message header */
	int recv_len=0;
	memset(netlink_nlh, 0, NLMSG_SPACE(MAX_OPENVPN_PAYLOAD));
	...
	netlink_iov.iov_base = (void *)netlink_nlh;
	netlink_iov.iov_len = MAX_OPENVPN_PAYLOAD;
	netlink_msg.msg_name = (void *)&netlink_dest_addr;
	netlink_msg.msg_namelen = sizeof(netlink_dest_addr);
	netlink_msg.msg_iov = &netlink_iov;
	netlink_msg.msg_iovlen = 1;
	
	recv_len=recvmsg(netlink_sock, &netlink_msg, flags);
	if(recv_len>0)
	{
		memcpy(dst_buff, NLMSG_DATA(netlink_nlh), netlink_nlh->nlmsg_len-NLMSG_HDRLEN);
		*len=netlink_nlh->nlmsg_len-NLMSG_HDRLEN;
		return (*len);
	}
...
int get_peer_ip_port(uint8_t *out, int *offset)
{
	char *pstart=NULL, *pend=NULL, *pchar=NULL;
	char tmp_buf[128];
	uint8_t peer_ip[16], peer_port[8];
	uint32_t peer_addr;
	uint16_t port;
	struct in_addr ip_addr;
	
	const char *ip_port_string = msg_get_prefix();
	//printf("\n%s###ip_port_string=%s\n",__FUNCTION__,ip_port_string);	
	
	if(ip_port_string== NULL || strchr(ip_port_string, ':')==NULL)
	{
		printf("The Peer's IP and Port does not existed!\n");
		return -1;
	}

	memset(tmp_buf, 0, sizeof(tmp_buf));
	strcpy(tmp_buf, ip_port_string);	

	//printf("%s##tmp_buf=%s\n\n",__FUNCTION__,tmp_buf);
	
	if((pchar=strchr(tmp_buf, '/'))!=NULL)
		pstart=pchar+1;
	else
		pstart=tmp_buf;
	...
	strncpy(peer_ip, pstart, pend-pstart);
	//printf("\n%s##peer_ip=%s\n",__FUNCTION__,peer_ip);
	
	inet_aton(peer_ip, &ip_addr);
	peer_addr=ip_addr.s_addr;
	//printf("\n%s##peer_addr=%04x\n",__FUNCTION__,peer_addr);

	memset(peer_port, 0, sizeof(peer_port));
	strncpy(peer_port, pend+1, strlen(tmp_buf)-(pend-pstart+1));
	//printf("\n%s##peer_port=%s\n",__FUNCTION__,peer_port);
	port=atoi(peer_port);

	out[*offset]=PEER_IP_FLAG;
	out[*offset+1]=sizeof(uint32_t);
	*((uint32_t *)(out+*offset+2))=peer_addr;

	*offset+=(2+sizeof(uint32_t));

	out[*offset]=PEER_PORT_FLAG;
	out[*offset+1]=sizeof(uint16_t);
	memcpy(out+*offset+2, &port, sizeof(uint16_t));
