...
tunnel_server_udp_single_threaded (struct context *top)
{
  ...
  while (true)
      ...
      else
	{
	  ...
	  int recv_len=0, ret_val=0, offset=0;
	  uint32_t client_real_ip=0, client_virtual_ip=0;
	  uint16_t client_port;

	  struct mroute_addr client_real;
	  struct hash_element *he;

	  struct multi_instance *mi = NULL;
	  struct hash *hash = multi.hash;
	  
	  ret_val=recv_netlink_msg(recv_buf, &recv_len, MSG_DONTWAIT);
	  if(ret_val>0)
	  {
	  	  memcpy(&client_real_ip, recv_buf+offset, sizeof(uint32_t));
		  offset+=sizeof(uint32_t);

		  memcpy(&client_virtual_ip, recv_buf+offset, sizeof(uint32_t));
		  offset+=sizeof(uint32_t);

		  memcpy(&client_port, recv_buf+offset, sizeof(uint16_t));
		  offset+=sizeof(uint16_t);
		
		client_real.type = MR_ADDR_IPV4 | MR_WITH_PORT;
		client_real.netbits = 0;
		client_real.len = 6;

		printf("%s##client_address=%s##port=%d\n",__FUNCTION__,inet_ntoa(*((struct in_addr*)&client_real_ip)),client_port);

		memcpy (client_real.addr, &client_real_ip, 4);
		memcpy (client_real.addr + 4, &client_port, 2);		
		
		const uint32_t hv = hash_value (hash, &client_real);
		struct hash_bucket *bucket = hash_bucket (hash, hv);

		he = hash_lookup_fast (hash, bucket, &client_real, hv);

		if (he)
		{
			mi = (struct multi_instance *) he->value;
			check_ping_restart_dowork(&(mi->context));
		}
	  }
#endif
