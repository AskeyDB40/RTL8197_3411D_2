...
void parse_rdnss (const struct nd_opt_hdr *opt, unsigned int ifindex)
{
	...
	if(last_servers.count == servers.count)
	{
		for(i=0;i<servers.count;i++)
		{
			if(memcmp(&last_servers.list[i].addr,&servers.list[i].addr,sizeof(servers.list[i].addr))!=0)
			{
				system("echo \"not update\">/var/rdnssd_dnss_need_update");				
				memcpy(&last_servers,&servers,sizeof(servers));
			}
		}
	}
	else
...
void parse_dnssl (const struct nd_opt_hdr *opt, unsigned int ifindex)
{
	...
	if(last_domains.count == domains.count)
	{
		for(i=0;i<domains.count;i++)
		{
			if(memcmp(&last_domains.list[i].domain,&domains.list[i].domain,sizeof(domains.list[i].domain))!=0)
			{   
				system("echo \"not update\">/var/rdnssd_dnsl_need_update");				
				memcpy(&last_domains,&domains,sizeof(domains));
			}
		}
	}
	else
