...
int send_connect_ack(RTK_SC_CTXp pCtx)
{
	...
	ack_msg.device_ip = intaddr; // intaddr is already big endian via ioctl. no need to transfer endian	
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if((pCtx->sc_wlan_status == 0) && (pCtx->sc_status == 0))
		{
			...
			if ( pCtx->sc_debug==2 )
			{
				printf("accept_control is %d, pCtx->sc_config_suncess is %d, pCtx->sc_wlan_status is %d\n",accept_control, pCtx->sc_config_success,pCtx->sc_wlan_status );
			}
		...
		if(1)//(pCtx->sc_wlan_status == 1)
		{
			...
			if (selret && FD_ISSET(sockfd_scan, &fds)) 
			{
				...
				else if(pCtx->sc_wlan_status == 1)
				{
					...
					pMsg->device_type = ntohs(pMsg->device_type);
					switch(pMsg->flag)
					{
						case SC_SUCCESS_ACK:
						...
						case SC_SCAN:
							...
							{
								if(pCtx->sc_debug == 2)
									printf("receive remove success ack, but not removed really!\n");
								break;
							}
						...
						case SC_SUCCESS_ACK:
							if((pCtx->sc_send_ack == SC_SUCCESS_ACK) || (pCtx->sc_send_ack == SC_SUCCESS_IP))
							{
								if(pCtx->sc_ip_status != SC_DHCP_GETTING_IP)
								{
									...
									//if(pCtx->sc_save_profile == 0)
									pCtx->sc_save_profile = 1; // to save in flash
									pCtx->sc_send_ack = 0;
									pCtx->sc_config_success = 1;
			...
			{
				accept_control = 1;
				// printf("1449, accept_control is %d\n", accept_control);
			...
			{				
				...
				//printf("enter control...sc_config_success is %d,pCtx->sc_send_ack is %d\n",pCtx->sc_config_success, pCtx->sc_send_ack);
				if(pCtx->sc_config_success == 1)
				{
					switch(pCtx->sc_send_ack)
					{
						...
						case SC_DEL_PROFILE:
							...
							if ( pCtx->sc_debug == 2 )
								printf("accept_control=1, then Remove_end.\n");
		...
		if( pCtx->sc_wlan_status== 0 && pCtx->sc_send_ack != SC_DEL_PROFILE ) //wlan  not connected
		{
			...
			if(pCtx->sc_status >= 10)
			{
				...
				{
					sprintf(buf, "iwpriv %s set_mib sc_status=0", pCtx->sc_wlan_ifname); // restart
					system(buf);
		...
		else // connected
		{
			...
			{
				pCtx->sc_linked_time++;
				if(pCtx->sc_linked_time == 1) // first time
				{
					reinit= 0;
					//pCtx->sc_save_profile = 0;
