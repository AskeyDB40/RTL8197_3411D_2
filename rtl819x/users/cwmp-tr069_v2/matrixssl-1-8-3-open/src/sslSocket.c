...
*/
	...
	/* bind to TR-069 interface if it's specified */
	if(gWanIfSpecified==1 && strlen(tr069WanIfName)!=0 )
	{
		CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("bind socket to %s\n", tr069WanIfName));
    	if(setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, tr069WanIfName, strlen(tr069WanIfName))<0){
			CWMPDBG_FUNC(MODULE_CORE, LEVEL_INFO, ("%s bind socket to %s fail! %d\n",__FUNCTION__, tr069WanIfName,errno));
			return INVALID_SOCKET;
    	}
	}
...
int sslConnect(sslConn_t **cpp, SOCKET fd, sslKeys_t *keys, 
			   ...
			   int (*certValidator)(sslCertInfo_t *t, void *arg), void *cb_data)
{
	sslConn_t	*conn;
...
*/
	...
	matrixSslSetCertValidator(conn->ssl, certValidator, cb_data);
...
*/
	if (rc == 0) {
		...
		{ 
			fprintf( stderr, "sslDoHandshake: skip EINTR/EAGAIN/EWOULDBLOCK(%d)\n", status );
			goto readMore;
		}
		goto error;
	}
...
*/
	case SSL_SEND_RESPONSE:
#ifdef _CHECKTO_
		{
			int ch_st, ch_ret;
			ch_st=0;
			ch_ret = check_wsk_timeout( cp->fd, &ch_st );
			if( ch_ret==1 ) //timeout
			{
				*status = SSLSOCKET_EOF;
				goto readError;
		...
		if (bytes == SOCKET_ERROR) {
			...
			*status = 0;
			//jiunming, set bytes=0 to continue
			bytes=0;
		}
		cp->inbuf.start += bytes;
		if (cp->inbuf.start < cp->inbuf.end) {
...
*/
	...
	if (rc == SOCKET_ERROR) {
		*status = getSocketError();
		...
		if( (*status!=EINTR) && (*status!=EAGAIN) && (*status!=EWOULDBLOCK) )
			return -1;
		//fprintf( stderr, "sslWrite: skip EINTR/EAGAIN/EWOULDBLOCK(%d)\n", *status );
		//reset status,rc values, for next call()...
		*status = 0;
		rc=0;
#else
		return -1;
