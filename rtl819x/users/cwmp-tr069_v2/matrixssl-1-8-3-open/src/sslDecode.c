...
//jiunming
static unsigned char bMSSLAllowSelfSignedCert=1;
void matrixSslSetAllowSelfSignedCert( unsigned char p)
{
	if(p) 
	  bMSSLAllowSelfSignedCert=1;
	else
	  bMSSLAllowSelfSignedCert=0;
}
...
*/
		...
		//matrixStrDebugMsg("new matrixStrDebugMsg\n", NULL);
		if( (bMSSLAllowSelfSignedCert!=0) && (subjectCert->valid<0) ) { //jiunming
			matrixStrDebugMsg("Try performing self-signed CA test\n", NULL);
			if (matrixX509ValidateCert(ssl->hsPool, subjectCert, 
					NULL,
					&subjectCert->valid) < 0) {
				ssl->err = SSL_ALERT_BAD_CERTIFICATE;
				matrixStrDebugMsg("Error validating certificate(self-signed CA test)\n", NULL);
				return SSL_ERROR;
			}
		}
...
#if 1
			{//jiunming
				unsigned char *pStartDN=c;
				DNattributes_t CurDNAttribute;
				memset( &CurDNAttribute, 0, sizeof(CurDNAttribute) );
				if( getDNAttributes( ssl->hsPool, &pStartDN, certLen, &CurDNAttribute )<0 )
				{
					fprintf( stderr, "<%s:%d>getDNAttributes error\n", __FUNCTION__, __LINE__ );
				}else{
					if( ssl->keys && ssl->keys->cert.certBin )
					{
						sslRsaCert_t	*currCert=NULL;
						
						if (matrixX509ParseCert(ssl->hsPool, ssl->keys->cert.certBin, ssl->keys->cert.certLen, &currCert) < 0)
						{
							fprintf( stderr, "<%s:%d>call matrixX509ParseCert() error\n", __FUNCTION__, __LINE__ );
						}else{
							if(currCert &&
								memcmp(CurDNAttribute.hash, currCert->issuer.hash, sizeof(CurDNAttribute.hash) )==0)
							{
								ssl->sec.certMatch = 1;
								fprintf( stderr, "<%s:%d>hash the same==>match CA DN\n", __FUNCTION__, __LINE__ );
							}
							else
								fprintf( stderr, "<%s:%d>hash NOT the same==>No match CA DN\n", __FUNCTION__, __LINE__ );
						}
						
						if(currCert) matrixX509FreeCert(currCert);
					}
					psFreeDNStruct( &CurDNAttribute );
				}
			}
#endif
			c += certLen;
