...
/*ping_zhang:20081217 END*/
	...
	if(instnum==1){
		...
		sprintf(ifname, "%s","eth0");
#else
	//LANDevice.{i}.LANEthernetInterfaceConfig. should be L3 LAN device
	//so it should be "br0" here ?
...
int getLANEthConf(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
	...
	char ifname[IFNAMSIZ];
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
	const char *multi_lan_dev[4] = {"eth0", "eth2", "eth3", "eth4"};
#endif
...
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
	sprintf(ifname, multi_lan_dev[instnum-1]);
#else
	sprintf(ifname, "eth0");
#endif
