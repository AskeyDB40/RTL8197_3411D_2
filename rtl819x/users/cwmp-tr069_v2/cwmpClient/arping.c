	...
	unsigned int *cntOK, unsigned int *cntFail, unsigned int *timeAvg, unsigned int *timeMin, unsigned int *timeMax, unsigned int needWaitRsp) 
{
	...
	fd_set rset;	
	int sock, bufsize, ret, cnt, recv_buf_size;
	//unsigned int attempt;
    /* uSequence and pingid are both host endianness */
	unsigned short uSequence = 0, uExpected, pingid;
	...
	while (count > 0) {
		...
		icmppkt->icmp_seq = htons(uSequence);
		icmppkt->icmp_id = htons(pingid);
