...
int getDHCPClient( int id,  char *ip, char *mac, char *liveTime )
{
	...
	if(p->expires == 0xffffffff)
		sprintf(liveTime,"%d", 0);
	else
		snprintf(liveTime, 10, "%lu", (unsigned long)ntohl(p->expires));
	return 0;
}
...
int getHostEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
	...
	getDHCPClient( id-1, ipAddr, macAddr, leaseTime );
	livetime = atoi(leaseTime);
