...
#endif
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("Send total %d bytes:\n", n));
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("\n%.*s\n", n, s));
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("Send packet dump end\n"));
#ifdef WITH_OPENSSL
      if (soap->ssl)
...
{ if (!ssl_init_done)
    soap_ssl_init();
  if (!soap->ctx)
  { if (!(soap->ctx = SSL_CTX_new(SSLv23_method()))) //jiunming, SSLv23_method
...
ssl_verify_callback(int ok, X509_STORE_CTX *store)
{
  ...
  { char data[256];
    ...
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_WARNING, ("certificate issuer %s\n", data));
    X509_NAME_oneline(X509_get_subject_name(cert), data, sizeof(data));
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_WARNING, ("certificate subject %s\n", data));
  }
#endif
...
soap_bind(struct soap *soap, const char *host, int port, int backlog)
{
  ...
  if(gWanIfSpecified==1 && strlen(tr069WanIfName)!=0 )
	{
    	if(setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_BINDTODEVICE, tr069WanIfName, strlen(tr069WanIfName))){
			soap->errnum = soap_socket_errno;
    		soap_set_receiver_error(soap, tcp_error(soap), "setsockopt failed in soap_bind()", SOAP_TCP_ERROR);
			 return SOAP_INVALID_SOCKET;
    	}
...
{ DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Close socket %d\n", (int)fd));
  CWMPDBG_FUNC(MODULE_CORE, LEVEL_INFO, ("close socket %d\n", fd));
...
{ soap_wchar c;
  ...
  *soap->endpoint = '\0';
  ...
  if ((int)c == EOF)
    return soap->error = SOAP_EOF;
  soap_unget(soap, c);
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("Receive total %d bytes:\n", soap->buflen));
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("\n%.*s\n",soap->buflen, soap->buf));
	CWMPDBG_FUNC(MODULE_CORE, LEVEL_DETAILED, ("Receive packet dump end\n"));
#ifndef WITH_NOHTTP
...
{ char header[SOAP_HDRLEN], *s;
  ...
  *soap->endpoint = '\0';
  ...
  { if (soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
    ...
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_INFO, ("****************http status*******************\n"));
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_INFO, ("%s\n", soap->msgbuf));
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_INFO, ("**********************************************\n"));
#endif
...
{ return soap_set_error(soap, soap->version == 2 ? "SOAP-ENV:Sender" : "Client", NULL, faultstring, faultdetail, soaperror);
}
#endif
...
{ return soap_set_error(soap, soap->version == 2 ? "SOAP-ENV:Receiver" : "Server", NULL, faultstring, faultdetail, soaperror);
}
#endif
...
{ if (soap_check_state(soap))
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_WARNING, ("Error: soap struct not initialized\n"));
  else if (soap->error)
  { const char *c, *v = NULL, *s, **d;
    ...
    CWMPDBG_FUNC(MODULE_CORE, LEVEL_WARNING, ("[%s:%d]%s%d fault: %s [%s]\t\"%s\"\tDetail: %s\n", __FUNCTION__, __LINE__,soap->version ? "SOAP 1." : "Error ", soap->version ? (int)soap->version : soap->error, c, v ? v : "no subcode", s ? s : "[no reason]", d && *d ? *d : "[no detail]"));
  }
}
#endif
#endif
