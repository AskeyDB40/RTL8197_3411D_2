...
#if 1 /*jiunming*/
    if (qop)
      sprintf(soap->tmpbuf, "Digest username=\"%s\", realm=\"%s\", nonce=\"%s\", uri=\"%s\", qop=\"%s\", nc=%s, cnonce=\"%s\", response=\"%s\", algorithm=\"MD5\"", soap->userid, soap->authrealm, data->nonce, soap->path, qop, ncount, cnonce, response);
    else
