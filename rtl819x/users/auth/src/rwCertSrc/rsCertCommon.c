...
int updateCertAreaHeader(const unsigned char certFlag)
{
	...
	char mtd_block_name[64]={0};

	ret=rtl_name_to_mtdblock(MTD_NAME, mtd_block_name);
	if(ret!=1)
	{		
		ERR_PRINT("%s(%d): get mtdblock fail.\n",__FUNCTION__, __LINE__);//Added for test
		toRet=FAILED;
		goto err;
	}
	...
	fh = open(mtd_block_name, O_RDWR | O_SYNC);

	if ( fh == -1 ) 
	{
		ERR_PRINT("%s(%d): open %s error.\n",__FUNCTION__, __LINE__, mtd_block_name);//Added for test
		toRet=FAILED;
		goto err;
	}
...
{
	int fh=-1, fd=-1;		// fh: /dev/mtdblock0 handler; fd: srcfile handler
...
//	char tmpBuf[USER_CERT_DIR_MAX_LEN+FILE_NAME_MAX_LEN];
	...
	//To read flash
	//fh = open(FLASH_DEVICE_NAME0, O_RDONLY); 
	fh = open(mtd_block_name, O_RDONLY);
	if ( fh == -1 ) 
	{
		ERR_PRINT("open %s error.\n", mtd_block_name);//Added for test
		toRet=FAILED;
