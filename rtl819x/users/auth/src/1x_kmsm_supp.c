	...
	if(pSupp_kmsm->bWaitForPacket && pGlobal->supp_kmsm->bAuthProgressing)
	{
		if(pSupp_kmsm->TimeoutCtr <= pSupp_kmsm->PairwiseUpdateCount )
		{
			if(pSupp_kmsm->TimeoutCtr >= 1)
			{
				...
				lib1x_nal_send( pGlobal->TxRx->network_supp, (char *)pGlobal->supp_pae->sendBuffer, pGlobal->EAPOLMsgSend.Length );
...
void lib1x_suppsm_capture_control( Supp_Global * pGlobal, lib1x_nal_intfdesc_tag * nal, lib1x_packet_tag * spkt )
{
	...
	case DOT11_EVENT_ASSOCIATION_IND:
		...
		if (pGlobal->auth->UsePassphrase) {
			...
			if (strcmp((char *)pGlobal->auth->RSNVariable.ssid, (char *)ssid)) {
				strcpy((char *)pGlobal->auth->RSNVariable.ssid, (char *)ssid);			
				PasswordHash((char *)pGlobal->auth->RSNVariable.PassPhrase, strlen((char *)pGlobal->auth->RSNVariable.PassPhrase),
					(unsigned char *)pGlobal->auth->RSNVariable.ssid, strlen((char *)pGlobal->auth->RSNVariable.ssid), pGlobal->auth->RSNVariable.PassPhraseKey);
				memcpy(pGlobal->supp_kmsm->PMK,
		...
		if ( pGlobal->auth->RSNVariable.WPA2Enabled) {
			...
			CalcPMKID(
					(char *)supp_kmsm->PMKID,
					(char *)supp_kmsm->PMK, 	 // PMK
					pAssoInd->MACAddr,   // AA
						(char *)pGlobal->TxRx->oursupp_addr, // SPA
						(pGlobal->AuthKeyMethod==DOT11_AuthKeyType_802_1X_SHA256)); 
			} else
#endif //CONFIG_IEEE80211W
			{
				CalcPMKID(
						...
						(char *)supp_kmsm->PMK, 	 // PMK
						pAssoInd->MACAddr,   // AA
						(char *)pGlobal->TxRx->oursupp_addr
#ifdef CONFIG_IEEE80211W
						,(pGlobal->AuthKeyMethod==DOT11_AuthKeyType_802_1X_SHA256)
#endif /* CONFIG_IEEE80211W */
					); // SPA
...
//remove
			...
			CalcPTK(pGlobal->EAPOLMsgRecvd.Octet, pGlobal->EAPOLMsgRecvd.Octet + 6,
				...
				pGlobal->supp_kmsm->PMK, PMK_LEN, pGlobal->supp_kmsm->PTK
#ifdef CONFIG_IEEE80211W
				, (pGlobal->AuthKeyMethod == DOT11_AuthKeyType_802_1X_SHA256)?48:PTK_LEN_TKIP
				, (pGlobal->AuthKeyMethod == DOT11_AuthKeyType_802_1X_SHA256)
#else
				, PTK_LEN_TKIP
#endif /* CONFIG_IEEE80211W */
...
void lib1x_skmsm_EAPOLKeySend(Supp_Global * pGlobal)
{
	...
	lib1x_nal_send( pGlobal->TxRx->network_supp, (char *)supp_pae->sendBuffer, pGlobal->EAPOLMsgSend.Length );
