...
int isMissArg(char *buf,int len,char *arg,int arg_len);
#endif
...
void UpnpSpecific()
{
	SearchEntry(60,"TCP","255.255.255.255.",60,"UCTT port description",1,1);
    upnp_redirect(60, "255.255.255.255", 60, "TCP", "UCTT port description",1);
}
#endif
...
SetConnectionType(struct upnphttp * h)
{
	...
	if(wan_uptime != 0){
		SoapError(h, 703, "InactiveConnectionStateRequired");
	}
	else
...
//	syslog(LOG_INFO, "AddportMapping Enabled=%d, %s, description : %s",atoi(bool_enabled), int_ip, desc);
	...
	iport = (unsigned short)atoi(int_port);
        ...
        strcpy(str,int_ip);
#ifdef CONFIG_CERTIFICATION
	r=SearchEntry(eport,protocol,int_ip,iport,desc,atoi(bool_enabled),0);
	if(r >=0){
...
static void GetGenericPortMappingEntry(struct upnphttp * h)
{
	...
	struct NameValueParserData data;
	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data);
	str = GetValueFromNameValueList(&data, "NewPortMappingIndex");
	if(!str){
		SoapError(h, 402, "Invalid Args");
		ClearNameValueList(&data);
		return;
	...
	num_index = upnp_get_portmapping_number_of_entries();
#endif
	index = atoi(GetValueFromNameValueList(&data, "NewPortMappingIndex"));
#ifdef CONFIG_CERTIFICATION
	if(index < 0 || index > (num_index-1)){
	...
	else
	{
		...
		if(rr >=0){
			bodylen = snprintf(body, sizeof(body), resp, (unsigned int)eport,
		                   protocol, (unsigned int)iport, iaddr, entry[rr].desc);
		}else
#endif
			bodylen = snprintf(body, sizeof(body), resp, (unsigned int)eport,
