...
/* The amount of time before advertisements
   will expire */
#ifndef CONFIG_CERTIFICATION
#define DEFAULT_ADVR_EXPIRE 180
#else
#define DEFAULT_ADVR_EXPIRE 360
#endif
//Brad add 20081205
...
#else
			sprintf(uuidvalue[0], "uuid:%s", value);
#endif
		}	
							  ...
							  const char * host, unsigned short port)
{
	...
	//	"Location: http://%s:%u" ROOTDESC_PATH "\r\n"
		...
		host, (unsigned int)port);
#else
		host, (unsigned int)port,random_url);
#endif
...
/* not really an SSDP "announce" as it is the response
 * to a SSDP "M-SEARCH" */
static void SendSSDPAnnounce3(int s, struct sockaddr* sockname,
                              const char * st, int st_len,
							  const char * host, unsigned short port,int index)
{
	int l, n;
	char buf[512];
	char date[30];
	...
	l = snprintf(buf, sizeof(buf), "HTTP/1.1 200 OK\r\n"
		...
		"\r\n",
		st_len, st,(index==-1?"":"1"),
		uuidvalue[(index==-1?0:index)], st_len, st,(index==-1?"":"1"),
		date,
#ifndef CONFIG_RANDOM_URL
		host, (unsigned int)port);
#else
...
#else
static const struct knownService known_service_types[] =
{
	//{"upnp:rootdevice",0},
	{"urn:schemas-upnp-org:device:InternetGatewayDevice:",0},
...
#ifdef ENABLE_6FC_SERVICE
	{"urn:schemas-upnp-org:service:WANIPv6FirewallControl:",2},
#endif
	//"urn:schemas-upnp-org:service:Layer3Forwarding:",
	//"urn:schemas-microsoft-com:service:OSInfo:",
	{0,0}
...
//					"Location:http://%s:%d/%s.xml" "\r\n"
...
#else
					 "Location:http://%s:%d/%s" ROOTDESC_PATH "\r\n"
#endif
			...
			//		host, PICS_INIT_PORT, PICS_DESC_NAME,
					...
					host, PICS_INIT_PORT,
#else
					host, PICS_INIT_PORT, random_url,
#endif			
					uuidvalue, uuidvalue,
					(type==0?"alive":"byebye"));
		if(l<0)
                 ...
                 unsigned int lifetime, int ipv6)
{
	...
	for(j=0;j<2;j++){
		i =0;
	while(known_service_types[i].known_service_type)
	{
		...
		l=sprintf(bufr,
			...
			//	host, PICS_INIT_PORT, PICS_DESC_NAME,
				...
				host, PICS_INIT_PORT,random_url,
#endif			
				known_service_types[i].known_service_type, "1",
				uuidvalue[known_service_types[i].uuid_index], known_service_types[i].known_service_type,"1",
				(type==0?"alive":"byebye"));
		if(l<0)
		{
			syslog(LOG_ERR, "SendSSDPNotifies() snprintf error");
	...
	else if(i>0 && i <=2)/*two discovery message for embedded device*/{
		/*the first */
		usleep(100*1000);
		l=sprintf(bufr,
				"NOTIFY * HTTP/1.1\r\n"
			...
			//	host, PICS_INIT_PORT, PICS_DESC_NAME,
				...
				host, PICS_INIT_PORT,random_url,
#endif			
				uuidvalue[known_service_types[i].uuid_index],
				uuidvalue[known_service_types[i].uuid_index],
				(type==0?"alive":"byebye"));
		if(l<0)
		{
		...
		l=sprintf(bufr,
			...
			//	host, PICS_INIT_PORT, PICS_DESC_NAME,
				...
				host, PICS_INIT_PORT,random_url,
#endif				
				known_service_types[i].known_service_type, (i==0?"":"1"),
				uuidvalue[known_service_types[i].uuid_index], known_service_types[i].known_service_type, (i==0?"":"1"),
				(type==0?"alive":"byebye"));
		if(l<0)
		{
			syslog(LOG_ERR, "SendSSDPNotifies() snprintf error");
...
static void
SendSSDPGoodbyeNotifies(int s, const char * host, unsigned short type,
                 unsigned int lifetime, int ipv6)
{
#ifdef ENABLE_IPV6
	struct sockaddr_storage sockname;
	...
	bufr = (char *) malloc(512);
	if (bufr == NULL) {
		syslog(LOG_ERR, "SendSSDPGoodbyeNotifies: out of memory!");
		return;
	}
	memset(bufr, 0, 512);
	memset(&sockname, 0, sizeof(sockname));
...
int OpenAndConfNotifySocket(const char * addr)
{
	...
	{
		syslog(LOG_ERR, "setsockopt(udp_notify, IP_MULTICAST_TTL): %m");
		close(s);
		return -1;
	}
#endif
...
void ProcessSSDPRequest(int s, const char * host, unsigned short port)
{
	...
	{
//		printf("%s %s %d: bufr = %s \n",__FILE__,__FUNCTION__,__LINE__,bufr);
		i = 0;
		while(i<n)
		{
			...
			if((i < n - 4) && strncasecmp(bufr+i, "man:", 4) == 0){
				man = bufr+i+4;
				while(*man == ' ' || *man == '\t' && (man < bufr + n)) man++;
				while(man[man_len]!='\r' && man[man_len]!='\n' && (man + man_len < bufr + n)) man_len++;
			}
#endif
		}
#ifdef CONFIG_CERTIFICATION
		...
		if(st && st_len >0)
		{
			...
			while(known_service_types[i].known_service_type)
			{
				l = (int)strlen(known_service_types[i].known_service_type);
				if(l<st_len && (0 == memcmp(st, known_service_types[i].known_service_type, l)))
				{
					strncpy(ver,st+l,st_len-l);
					ver[st_len-l]='\0';
					if(!strcmp("1",ver))
					SendSSDPAnnounce3(s,(struct sockaddr*)&sendername, known_service_types[i].known_service_type, l, 
						announced_host, port,known_service_types[i].uuid_index);
					break;
				}
				i++;
			}
			...
			if(st_len==8 && (0 == memcmp(st, "ssdp:all", 8)))
			{
				...
				l = strlen("upnp:rootdevice");
				SendSSDPAnnounce3(s,(struct sockaddr*)&sendername,
					                  "upnp:rootdevice", l,
					                  announced_host, port,-1);
				for(i=0; known_service_types[i].known_service_type; i++)
				{
					l = (int)strlen(known_service_types[i].known_service_type);
					SendSSDPAnnounce3(s,(struct sockaddr*)&sendername,
					                  known_service_types[i].known_service_type, l,
					                  announced_host, port,known_service_types[i].uuid_index);
				}
				/* also answer for uuid */
				for(i = 0;i<3;i++)
				SendSSDPAnnounce2(s, (struct sockaddr*)&sendername, uuidvalue[i],strlen(uuidvalue[i]) ,
				                  announced_host, port);
				return;
			}
			l= strlen("upnp:rootdevice");
			if(st_len==l && (0 == memcmp(st, "upnp:rootdevice", l)))
			{
			...
			for(i = 0;i < 3;i++){
				l = (int)strlen(uuidvalue[i]);
				if(l==st_len && (0 == memcmp(st, uuidvalue[i], l)))
				{
					SendSSDPAnnounce2(s, (struct sockaddr*)&sendername, st, st_len, announced_host, port);
				}
			}
...
void miniupnpdShutdown()
{
...
#endif
    {
        snprintf(bufr, sizeof(bufr),
				 ...
				 known_service_types[i], (i==0?"":"1"),
#ifdef CONFIG_CERTIFICATION
                 uuidvalue[known_service_types[i].uuid_index], known_service_types[i], (i==0?"":"1"));
#else
				 uuidvalue, known_service_types[i], (i==0?"":"1"));	
#endif
        n = sendto(s, bufr, strlen(bufr), 0,
...
static int substr(char *docinpath, char *infile, char *docoutpath, char *outfile, char *str_from, char *str_to)
{
	...
	char *controlURL = "<controlURL>";
	char controlURL_random[32]={0};
	sprintf(controlURL_random,"%s/%s",controlURL,random_url);
	char controlURL_len = strlen(controlURL);
	char controlURL_random_len = strlen(controlURL_random);

	char *eventSubURL = "<eventSubURL>";
	char eventSubURL_random[32]={0};
	sprintf(eventSubURL_random,"%s/%s",eventSubURL,random_url);
	char eventSubURL_len = strlen(eventSubURL);
	char eventSubURL_random_len = strlen(eventSubURL_random);

	char *scpdURL = "<SCPDURL>";
	char scpdURL_random[20]={0};
	sprintf(scpdURL_random,"%s/%s",scpdURL,random_url);
	char scpdURL_len = strlen(scpdURL);
	char scpdURL_random_len = strlen(scpdURL_random);
...
#ifdef CONFIG_RANDOM_URL		
			else if(strncmp(buffi+i, controlURL, controlURL_len)==0){
				strcpy (buffo+j, controlURL_random);
				i += controlURL_len - 1;
				j += controlURL_random_len - 1;
			}
			else if(strncmp(buffi+i, eventSubURL, eventSubURL_len)==0){
				strcpy (buffo+j, eventSubURL_random);
				i += eventSubURL_len - 1;
				j += eventSubURL_random_len - 1;
			}
			
			else if(strncmp(buffi+i, scpdURL, scpdURL_len)==0){
				strcpy (buffo+j, scpdURL_random);
				i += scpdURL_len - 1;
				j += scpdURL_random_len - 1;
			}
#endif
			else
				*(buffo + j) = *(buffi + i);
...
static void AddToChain(const char *if_name, const char *ip)
{
	...
	//delete last rule
	if(stat(UPNP_INIT, &status) == 0){
		if(fp = fopen(UPNP_INIT,"r")){
			while(fgets(tmp,200,fp)){
				if(strstr(tmp,"MINIUPNPD") && strstr(tmp,"iptables")){
					system(tmp);
				}
			}
			fclose(fp);
	...
	sprintf(tmp, "iptables -t nat -I %s  -d %s -i %s -j MINIUPNPD",PREROUTING_CHAIN_NAME[wan_idx], ip, if_name);
	system(tmp);
#if defined(CONFIG_RTL8186_GR) || defined(CONFIG_RTL8186_GW)
	sprintf(tmpBuf,"echo -n \"iptables -t nat -D %s  -d %s -i %s -j MINIUPNPD\n\" > %s",PREROUTING_CHAIN_NAME[wan_idx], ip, if_name,UPNP_INIT);
	system(tmpBuf);
#endif
	sprintf(tmp, "iptables -t filter -I %s -i %s -o ! %s -j MINIUPNPD", FORWARD_CHAIN_NAME[wan_idx],if_name, if_name);
	system(tmp);
#if defined(CONFIG_RTL8186_GR) || defined(CONFIG_RTL8186_GW)
	sprintf(tmpBuf,"echo -n \"iptables -t filter -D %s -i %s -o ! %s -j MINIUPNPD\n\" >> %s",FORWARD_CHAIN_NAME[wan_idx],if_name, if_name,UPNP_INIT);
	system(tmpBuf);
#endif
...
#if defined(MULTI_WAN_SUPPORT)
		sprintf(file_name, "/var/system/multichain%d", wan_idx);
...
#if defined(MULTI_WAN_SUPPORT)
	char wan_if_name[MAX_LEN_IF_NAME];
#endif
	...
	//if (WANconnected > 0) {
		...
		//if (wisp_interface_num < 0) 
		{
			switch (wan_type)
			{
...
#endif
#if defined(MULTI_WAN_SUPPORT)
					sprintf(wan_if_name,"ppp%d",wan_idx);
					CheckInterfaceChanged(wan_if_name);
#else
					if(sessionSel==1)
						CheckInterfaceChanged(WAN_IF_2_1);
					else
				...
				default:
#if defined(MULTI_WAN_SUPPORT)
				sprintf(wan_if_name,"eth1.%d",wan_idx);
				CheckInterfaceChanged(wan_if_name);
#else
				if (wisp_interface_num < 0) 
...
int main(int argc, char * * argv)
{
	...
	for(i=1; i<argc; i++)
	{
		...
		case 'u':
#ifdef CONFIG_CERTIFICATION
			strncpy(uuidvalue[0]+5, argv[++i], strlen(uuidvalue[0]+5) + 1);
#else
...
//To generate different uuid for different device according to /tmp/eth1_mac_addr
...
#ifdef CONFIG_CERTIFICATION
			sprintf(uuidvalue[0],"uuid:82350000-4000-0000-0000-%s",tmpBuf);
#else
	...
	system("rm -f /tmp/eth1_mac_addr");
		...
		snprintf(random_url, RANDOM_URL_MAX_LEN, "%08lx", random()); //long->hex(16) output 
		//printf("%s %d:random_url = %s \n",__FILE__,__LINE__,random_url);
...
#ifdef CONFIG_CERTIFICATION
	SendSSDPGoodbyeNotifies(snotify, listen_addr,1,0,0);
#ifdef ENABLE_IPV6
	SendSSDPGoodbyeNotifies(snotify6,ipv6_addr_for_http_with_brackets,1,0,1);
#endif
#endif


#ifdef CONFIG_RANDOM_URL
#define RANDOM_INTERVAL 300 //randomize URL per 5mins
...
//		{
	...
	int interval = (int)difftime(end,start);
	if( interval%RANDOM_INTERVAL == 0 && interval > 0){ //randomize URL per 5mins
		srandom((unsigned)time(0));
		snprintf(random_url, RANDOM_URL_MAX_LEN, "%08lx", random()); //long->hex(16) output 
	//	printf("%s %d:random_url = %s \n",__FILE__,__LINE__,random_url);
		substr("/etc/linuxigd/","picsdesc.skl","/etc/linuxigd/","picsdesc.xml","!ADDR!", listen_addr);
...
#ifdef CONFIG_CERTIFICATION
					notify_interval=(ssdp_mx) ? ((ssdp_mx/2-10)/CHECK_WAN_INTERVAL-1):((DEFAULT_ADVR_EXPIRE/2-10)/CHECK_WAN_INTERVAL-1);
#else
