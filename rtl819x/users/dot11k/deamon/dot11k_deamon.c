...
unsigned char request_ie[] = {_BSS_LOAD_IE_, _HT_CAP_, _MOBILITY_DOMAIN_IE_};
int debug_mode = 0;
...
struct dot11k_neighbor_bss_load dot11k_neighbor_bssload[MAX_NEIGHBOR_NUM];
...
static int colloect_neighbor_report(struct dot11k_beacon_measurement_report * beacon_report)
{
    ...
    if(i == dot11k_neighbor_num ) { /* not found */
        if(dot11k_neighbor_num == MAX_NEIGHBOR_NUM) /*full*/
        return 1;
    dot11k_neighbor_num++;
...
#ifdef CONFIG_IEEE80211V
    dot11k_neighbor_list[i].subelemnt.subelement_id = _WNM_BSS_TRANS_CANDIDATE_PREFRENCE_;
    dot11k_neighbor_list[i].subelemnt.len = 1;
    dot11k_neighbor_list[i].subelemnt.preference = 0;
#endif
    dot11k_neighbor_bssload[i].sta_count = 0;
    dot11k_neighbor_bssload[i].channel_utilization = 0;
    ...
    while(len + 2 <= beacon_report->subelements_len)
    {
        ...
        if(subelement_id == _FRAME_BODY_SUBIE_)
        {
            ...
            while(len + 2 <= beacon_report->subelements_len)
            {
                ...
                else if(element_id == _BSS_LOAD_IE_) { 
                    dot11k_neighbor_bssload[i].sta_count = le16toh(*(unsigned short *)&beacon_report->subelements[len + 2]);
                    dot11k_neighbor_bssload[i].channel_utilization = beacon_report->subelements[len + 4];
                }
                len += 2 + element_len;                    
            }
        }
...
void TimerFunc()
{
    ...
    for(i = 0; i < dot11k_neighbor_num; i++)
    {
        sprintf((char *)tempbuf, "echo \"add %02x%02x%02x%02x%02x%02x 0x%04X %d %d %d ssid \\\"%s\\\" bssload %d %d\"",
                ...
                iface_ssid,
                dot11k_neighbor_bssload[i].sta_count,
                dot11k_neighbor_bssload[i].channel_utilization);
...
int main(int argc, char *argv[])
{
    ...
    time_interval = DEFAULT_TIMER_INTERVAL;
