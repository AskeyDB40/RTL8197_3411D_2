...
#ifdef _AIPC_CPU_
spinlock_t lock_aipc_osal_2cpu;
int lock_aipc_owner_2cpu = -1;

spinlock_t lock_aipc_osal_2dsp;
int lock_aipc_owner_2dsp = -1;
#endif
#endif
...
#ifdef CONFIG_VOIP_HOST_SMP_LOCK
#ifdef _AIPC_CPU_
	spin_lock_init(&lock_aipc_osal_2cpu);
	spin_lock_init(&lock_aipc_osal_2dsp);
#endif
#endif
}
...
aipc_int_2dsp_hiq_enqueue( u32_t int_id )
{
	...
	AIPC_OSAL_INTERRUPT_DISABLE_2DSP(state);
...
#ifdef INTQ_LOCK
	AIPC_OSAL_INTERRUPT_ENABLE_2DSP(state);
