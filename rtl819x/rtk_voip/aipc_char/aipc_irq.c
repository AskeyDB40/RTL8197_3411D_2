...
static int 
aipc_check_pending(void)
{
	volatile u32_t status=0;
	int need_break=0;
	
#if defined(CONFIG_RTL8686_SHM_NOTIFY)
	need_break = 1;
#else
	#if defined(CONFIG_RTL8685SB)
		status = REG32(R_AIPC_CPIS);
...
void aipc_2cpu_rx_tasklet(unsigned long task_priv)
{
...
#ifdef AIPC_SMP_TIME_DEBUG
	int cpu = smp_processor_id();
	unsigned long start , stop , offset; 
	ktime_t k_start , k_stop , k_offset;
	unsigned long k_offset_ms;
	...
	k_stop      = ktime_get();
	offset      = stop - start; 
	k_offset    = ktime_sub(k_stop , k_start);
	k_offset_ms = ktime_to_ms(k_offset);
	if( k_offset_ms > CHECK_KTIME_CNT ){
		printk("%s(%d) %d %lu %lu %lu\n"         , __FUNCTION__ , __LINE__ , cpu , offset , start , stop);
		printk("%s(%d) %d %llu %llx %llx %llx\n" , __FUNCTION__ , __LINE__ , cpu , k_offset_ms , k_offset , k_start , k_stop);		
	}
#endif
}
...
/*******************************************************************************
*	AIPC interrupt assert/deassert mask/unmask functions
*******************************************************************************/
...
aipc_int_assert(T_COP cop)
{
	...
	return 0;
#else /* !defined(CONFIG_RTL8686_SHM_NOTIFY) */
#if defined(CONFIG_RTL8685SB) || defined(CONFIG_AIPC_PLATFORM_0822_5281)
	if (cop==T_CPU){
		REG32(R_AIPC_CPIT) |= P1_TRIGGER;
	}
	else if (cop==T_DSP){
		REG32(R_AIPC_CPIT) |= P0_TRIGGER;
	}
	else{
		printk("not supported type\n");
	}
...
#if defined(CONFIG_RTL8685SB) || defined(CONFIG_AIPC_PLATFORM_0822_5281)
	if (cop==T_CPU){
		REG32(R_AIPC_CPIC) |= P1_CLEAR;
	}
	else if (cop==T_DSP){
		REG32(R_AIPC_CPIC) |= P0_CLEAR;
	}
	else{
		printk("not supported type\n");
	}
...
#if 0
	if (cop==T_CPU){
		GIC_SET_INTR_MASK( GIC_EXT_CPU0_STATUS );
	}
	else if (cop==T_DSP){
		GIC_SET_INTR_MASK( GIC_EXT_5281_STATUS );
	}
	else{
		printk("not supported type\n");
	}
#endif	
#else /* !defined(CONFIG_RTL8685SB) */
	if (cop==T_CPU){
#if 0
