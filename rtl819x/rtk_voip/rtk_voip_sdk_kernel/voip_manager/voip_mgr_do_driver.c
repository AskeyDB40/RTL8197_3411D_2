...
#ifndef CONFIG_LINUX_KERNEL_2_6
...
//ericchung: stop dtmf callerid ring
...
/**
 * @ingroup VOIP_DRIVER_SLIC
 * @brief Message Waiting Indicator for neon lamp  
 * @see VOIP_MGR_SLIC_NEON_LAMP TstFskClid 
 */
int do_mgr_VOIP_MGR_SLIC_NEON_LAMP( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
    ...
    PRINT_MSG("VOIP_MGR_SLIC_NEON_LAMP:ch_id = %d, state = %d\n", stFskClid.ch_id, stFskClid.state);

    if ( 1 == Is_DAA_Channel(stFskClid.ch_id))
		return ret;

	SLIC_Set_Neon_Lamp_State(stFskClid.ch_id, stFskClid.state); 
...
{
#ifdef SUPPORT_DSCP
#if 1//def CONFIG_LINUX_KERNEL_2_6
#ifdef CONFIG_RTK_VOIP_QOS
