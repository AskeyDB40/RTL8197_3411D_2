...
int ZarlinkPortDetection(RTKLineObj *pLine, unsigned int *val)		
{
	return RTK_CALL_DEV_FUNC(StartLineTest, ((pLine), 3, val));
}

int ZarlinkLTROH(RTKLineObj *pLine, unsigned int *val)		
{
	return RTK_CALL_DEV_FUNC(StartLineROH, ((pLine), 2, val));
}

int ZarlinkLTLINEVOLTAGE(RTKLineObj *pLine, unsigned int *val)		
{
	return RTK_CALL_DEV_FUNC(StartLineVOLTAGE, ((pLine), 1, val));
}

int ZarlinkLTRESFLT(RTKLineObj *pLine, unsigned int *val)		
{
	return RTK_CALL_DEV_FUNC(StartLineRESFLT, ((pLine), 4, val));
}

#endif

int ZarlinkGetCalibrationCoeff(RTKLineObj *pLine, unsigned char *coeff)
{
	return RTK_CALL_DEV_FUNC(GetCalibrationCoeff, ((pLine), coeff));
}

int ZarlinkSetCalibrationCoeff(RTKLineObj *pLine, unsigned char *coeff)
{
	return RTK_CALL_DEV_FUNC(SetCalibrationCoeff, ((pLine), coeff));
}

int ZarlinkDoCalibration(RTKLineObj *pLine, unsigned char mode)
{
	return RTK_CALL_DEV_FUNC(DoCalibration, ((pLine), mode));
}

#ifdef CONFIG_RTK_VOIP_SLIC_ZARLINK_886_SERIES
int ZarlinkMeteringPulse
(	RTKLineObj *pLine,
	uint16 hz,
	uint16 onTime, 
	uint16 offTime, 
	uint16 numMeters)
{
	return RTK_CALL_DEV_FUNC(MeteringPulse, (pLine, hz, onTime, offTime, numMeters));
}
#endif
