...
#ifdef CONFIG_SMP
struct vpsys_spinlock VpSysSpinLock[VP_NUM_CRITICAL_SEC_TYPES];
#else
...
static void __init VpSysInitParams(void)
{
	int i;

	for (i=0; i<VP_NUM_CRITICAL_SEC_TYPES; i++) {
		VpSysSpinLock[i].func[0] = 0;
		VpSysSpinLock[i].owner = -1;
		VpSysSpinLock[i].flags = 0;
		VpSysSpinLock[i].depth = 0;
		memset(VpSysSpinLock[i].locked, 0, sizeof(VpSysSpinLock[i].locked));
		spin_lock_init(&VpSysSpinLock[i].lock);
	}
}

module_init(VpSysInitParams);
