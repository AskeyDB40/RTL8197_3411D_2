    ...
    */
    if(Si3226x_General_Configuration.batType == BO_DCDC_TRACKING)
        ...
        */
        ...
        if((pProslic->dcdc_polarity_invert)&&(Si3226x_General_Configuration.bomOpt == BO_DCDC_PMOS_BUCK_BOOST))
        {
...
#else
					printk("%sChannel %d : Type = %s\n", LOGPRINT_PREFIX, pProslic[k]->channel, "DAA");					
#endif
                }
				else
                {
...
#else
					printk("%sChannel %d : Type = %s\n", LOGPRINT_PREFIX, pProslic[k]->channel, "UNKNOWN");
#endif
                }
			}
	#endif
...
int Si3226x_Init (proslicChanType_ptr *pProslic, int size){
#ifdef REALTEK_PATCH_FOR_SILAB
#if 0
	printk("sizeof(Si3226x_Ring_Cfg) = %d\n", sizeof(Si3226x_Ring_Cfg));
	printk("sizeof(Si3226x_DCfeed_Cfg) = %d\n", sizeof(Si3226x_DCfeed_Cfg));
	printk("sizeof(Si3226x_Impedance_Cfg) = %d\n", sizeof(Si3226x_Impedance_Cfg));
	printk("sizeof(Si3226x_audioEQ_Cfg) = %d\n", sizeof(Si3226x_audioEQ_Cfg));
	printk("sizeof(Si3226x_hybrid_Cfg) = %d\n", sizeof(Si3226x_hybrid_Cfg));
	printk("sizeof(Si3226x_Zsynth_Cfg) = %d\n", sizeof(Si3226x_Zsynth_Cfg));
	printk("sizeof(Si3226x_PCM_Cfg) = %d\n", sizeof(Si3226x_PCM_Cfg));
	printk("sizeof(Si3226x_General_Cfg) = %d\n", sizeof(Si3226x_General_Cfg));
#endif
#if 0
	...
	printk("Ring size = %d\n", si3226x_ring_preset_size);
	printk("DCFeed size = %d\n", si3226x_dcfeed_preset_size);
	printk("Impedance size = %d\n", si3226x_impedance_preset_size);
#endif
#endif
...
#ifdef REALTEK_PATCH_FOR_SILAB
    	PRINT_MSG("Si3226x in Narrowband.\n");
#endif	
        regTemp = ReadReg(pProHW,pProslic->channel,DIGCON);
...
#ifdef REALTEK_PATCH_FOR_SILAB
	regTemp = ReadReg(pProHW,pProslic->channel,LINEFEED);
	printk("LINEFEED 0x%02x, newLinefeed 0x%02x\n", regTemp, newLinefeed);
	if ( ( regTemp & 0xf ) == newLinefeed )
		return RC_NONE;
#endif
...
#ifdef REALTEK_PATCH_FOR_SILAB
    LOGPRINT("ch %d Si3226x linefeed set 0x%x\n",pProslic->channel,newLinefeed);
#endif	
    return 0;
}
...
int Si3226x_RingStart (proslicChanType *pProslic){
    ...
    uInt8 lf = 0;
    	...
    	lf = ProSLIC_ReadReg(pProslic,LINEFEED);  
        if(lf & 0x04)
        {
#ifdef ENABLE_DEBUG
			if (pProslic->debugMode)
				LOGPRINT("Warning: Si3226x linefeed 0x%x is ringing now, ingnore RingStart\n",lf);
#endif
            return;
        }else
        {
#ifdef ENABLE_DEBUG
			if (pProslic->debugMode)
        	    LOGPRINT("check Si3226x linefeed is 0x%x\n",lf);
#endif
        } 
#endif
...
#ifdef ENABLE_DEBUG
			if (pProslic->debugMode)
				LOGPRINT("Warning: Si3226x linefeed 0x%x is acitve now, ingnore RingStop\n",lf);
#endif
            return;
        }else
        {
