...
extern spinlock_t lock_Reg;
extern spinlock_t lock_Ram;

extern int lock_Reg_owner;
extern int lock_Ram_owner;

#if 1
#define SPI_Reg_LOCK(x)     spin_lock_irqsave(&lock_Reg, (x))
#define SPI_Reg_UNLOCK(x)   spin_unlock_irqrestore(&lock_Reg, (x))
#define SPI_Ram_LOCK(x)     spin_lock_irqsave(&lock_Ram, (x))
#define SPI_Ram_UNLOCK(x)   spin_unlock_irqrestore(&lock_Ram, (x))
#else // debug version
#define SPI_Reg_LOCK(x)	\
	do { \
         if(lock_Reg_owner!=smp_processor_id()) \
             spin_lock_irqsave(&lock_Reg, (x)); \
         else \
             PRINT_R("[%s %s %d] recursion detection\n",__FILE__, __FUNCTION__,__LINE__); \
         lock_Reg_owner=smp_processor_id();\
     } while(0)

#define SPI_Reg_UNLOCK(x)	\
	do { \
         lock_Reg_owner=-1; \
         spin_unlock_irqrestore(&lock_Reg, (x)); \
     } while(0)


#define SPI_Ram_LOCK(x)	\
	do { \
         if(lock_Ram_owner!=smp_processor_id()) \
             spin_lock_irqsave(&lock_Ram, (x)); \
         else \
             PRINT_R("[%s %s %d] recursion detection\n",__FILE__, __FUNCTION__,__LINE__); \
         lock_Ram_owner=smp_processor_id();\
     } while(0)

#define SPI_Ram_UNLOCK(x)	\
	do { \
         lock_Ram_owner=-1; \
         spin_unlock_irqrestore(&lock_Ram, (x)); \
     } while(0)
#endif
...
#elif 1
	_rtl_spi_transRead8( pDev, regCtrl, regAddr, &reg_val );
#else
	//printk("(%d, %d)\n", portID, channel);
 	// .8138 (.2474)
...
#elif 1
	_rtl_spi_transWrite8( pDev, regCtrl, regAddr, data );
#else
	_rtl_spi_rawWrite(pDev, &regCtrl, 8);
	_rtl_spi_rawWrite(pDev, &regAddr, 8);
...
/*
#define VTESTNAME3 "slicReadID"

static int voip_add_test3_read(char *page, char **start, off_t off,
...
static int voip_add_test3_write(struct file *file, const char *buffer, 
                               unsigned long count, void *data)
{
	char tmp[128], result = 0;
	int t1 = 0;
	rtl_spi_dev_t pDev[0];
	
	memset(pDev, 0x0, sizeof(rtl_spi_dev_t));
...
static int __init voip_proc_add_test3_init(void)
{
	struct proc_dir_entry *voip_add_test_proc;

	voip_add_test_proc = create_proc_entry( VTESTNAME3, 0644, NULL);
	if (voip_add_test_proc == NULL) {
		remove_proc_entry(VTESTNAME3, NULL);
		printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
				VTESTNAME3);
		return -ENOMEM;
	}
	voip_add_test_proc->read_proc  = (read_proc_t *)voip_add_test3_read;
	voip_add_test_proc->write_proc  = (write_proc_t *)voip_add_test3_write;

	printk("/proc/%s created\n", VTESTNAME3);
	return 0;
}

static void __exit voip_proc_add_test3_exit( void )
{
	remove_proc_entry( VTESTNAME3, NULL );
}

module_init( voip_proc_add_test3_init );
module_exit( voip_proc_add_test3_exit );
*/
