...
**
** $Id: si_voice.c 4461 2014-07-16 15:39:20Z nizajerk $
**
** si_voice.c
**
** Author(s): 
...
int SiVoice_SWInitChan (SiVoiceChanType_ptr pChan,int channel,int chipType, SiVoiceDeviceType*pDeviceObj, SiVoiceControlInterfaceType *pCtrlIntf){
	...
	switch (chipType){
	...
	#elif defined CONFIG_RTK_VOIP_DRIVERS_SLIC_SI3217x_LCQC
			pChan->deviceId->chipType = SI32174;
			pChan->channelType = PROSLIC;
			pChan->bomOption = BO_DCDC_LCQCUK;
			printk("Set to BO_DCDC_LCQCUK\n");
	#else
			printk("Warning: need to set correct DC-DC structure for your SLLC type in %s - %s - line %d\n", __FILE__, __FUNCTION__, __LINE__);
	#endif
#endif			
		...
		case SI3226X_TYPE:
...
#else
			pChan->deviceId->chipType = SI32268;
			pChan->channelType = PROSLIC;//SI32261;
			...
			pChan->bomOption = BO_DCDC_PMOS_BUCK_BOOST;
			printk("Set to BO_DCDC_PMOS_BUCK_BOOST\n");
#elif defined (CONFIG_RTK_VOIP_DRIVERS_SLIC_SI3226x_TSS_ISO)
			pChan->bomOption = BO_DCDC_FLYBACK;
			printk("Set to TSS_ISO : BO_DCDC_FLYBACK\n");
#else
			printk("Warning: need to set correct DC-DC structure for your SLLC type in %s - %s - line %d\n", __FILE__, __FUNCTION__, __LINE__);
#endif
#endif			
