...
#define FEATURE_DMEM_STACK_CLI
	...
	!defined (CONFIG_RTK_VOIP_DRIVERS_PCM89xxD) && !defined (CONFIG_RTK_VOIP_DRIVERS_PCM89xxE) && !defined( CONFIG_RTK_VOIP_DRIVERS_PCM8881A ) 
...
#if defined (CONFIG_RTK_VOIP_DRIVERS_PCM89xxE) || defined (CONFIG_RTK_VOIP_PLATFORM_8686)
#define MAX_DSP_RTK_CH_NUM	8
#else
#define MAX_DSP_RTK_CH_NUM	16
#endif
#define MAX_DSP_RTK_SS_NUM	( MAX_DSP_RTK_CH_NUM * 2 )
#if defined (CONFIG_RTK_VOIP_DRIVERS_PCM89xxE) || defined (CONFIG_RTK_VOIP_PLATFORM_8686)
#define DSP_RTK_CH_NUM		8	// less or equal to MAX_DSP_RTK_CH_NUM
#else
...
#define CONFIG_DEFAULT_NEW_EC128	0
#endif
...
#elif 0
extern spinlock_t lock_voip;
#define save_flags(x) spin_lock_irqsave(&lock_voip, x)
#define restore_flags(x) spin_unlock_irqrestore(&lock_voip, x)
#else
extern int lock_voip_owner;
extern spinlock_t lock_voip;
#define save_flags(x)	\
	do { \
         if(lock_voip_owner!=smp_processor_id()) \
             spin_lock_irqsave(&lock_voip, (x)); \
         else \
             PRINT_R("[%s %s %d] recursion detection\n",__FILE__, __FUNCTION__,__LINE__); \
         lock_voip_owner=smp_processor_id();\
     } while(0)
#define restore_flags(x)	\
	do { \
         lock_voip_owner=-1; \
         spin_unlock_irqrestore(&lock_voip, (x)); \
     } while(0)
#endif
#endif
...
#define RFC2833TRUNKEVENT		// support rfc 2833 trunk event
#define SLICDUMPREG				// support slic dump register information
//#define INTER_RING	// for two channel interleave ring
