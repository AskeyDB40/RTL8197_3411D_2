...
/*support disable codec */
	websWrite(wp,
		"<td bgColor=#ddeeff width=85 rowspan=2>Disable</td>"
		);
...
#endif
{
	...
	else if (strcmp(argv[0], "sip_number")==0)
	{
		for (i=0; i< VOIP_PORTS; i++)
		{
			if(pVoIPCfg->ports[i].proxies[0].number != NULL)
				websWrite(wp, "<td>%s</td>", pVoIPCfg->ports[i].proxies[0].number);	
		}
	}
		...
		if (fread(buf, sizeof(buf), 1, fh) == 0) {
			...
			for (i=0; i< VOIP_PORTS; i++)
			{
				...
				else
				{
					...
					switch (p_reg_st) {
					case '0':
						websWrite(wp, "<td>%s</td>", "Not Registered");
						break;
					case '1':
						websWrite(wp, "<td>%s</td>", "Registered");
						break;
					case '2':
						websWrite(wp, "<td>%s</td>", "Registering");
						break;
					case '5':
						websWrite(wp, "<td>%s</td>", "Register Fail");
						break;
					case '3':
						websWrite(wp, "<td>%s</td>", "Register Fail");
						break;
					case '4':
						websWrite(wp, "<td>%s</td>", "VoIP Restart");
						break;
					default:
						websWrite(wp, "<td>%s</td>", "Error");
						break;
					}
				}
			}
	...
	else if (strcmp(argv[0], "proxy")==0)
	{
		...
		for (i=0; i<MAX_PROXY; i++)
		{
			...
			{
				if(i==0)
					websWrite(wp, "<p><b>Main Proxy</b>\n");
				else
					websWrite(wp, "<p><b>Backup Proxy</b>\n");
			}
			else
...
//sip Session
			...
			websWrite(wp,
				"<tr>\n" \
				"<td bgColor=#aaddff>Enable Session timer</td>\n" \
				"<td bgColor=#ddeeff><input type=checkbox id=sessionEnable name=sessionEnable%d %s>Enable</td>\n" \
				"</tr>\n",
				i, (pCfg->proxies[i].SessionUpdateTimer) ? "checked" : "");

			websWrite(wp,
						"<tr>\n" \
						"<td bgColor=#aaddff>Session Expire (sec)</td>\n" \
						"<td bgColor=#ddeeff>\n" \
						"<input type=text id=sessionExpiry name=sessionExpiry%d size=6 maxlength=5 value=%d></td>\n"
						"</tr>\n",
						i, pCfg->proxies[i].SessionUpdateTimer);
...
//			fprintf(stderr, "buf is %s.\n", buf);
			switch (buf[voip_port * MAX_PROXY + i]) {
				...
				case '5':
					...
					break;
				case '3':
					websWrite(wp, "%s", "Register Fail");
					break;
				case '4':
					websWrite(wp, "%s", "VoIP Restart...");
					break;
				default:
					websWrite(wp, "%s", "ERROR");
					break;
	...
	else if (strcmp(argv[0], "Call_Transfer")==0)
	{
		if(	pCfg->fxsport_flag & FXS_CALL_TRANSFER)
			websWrite(wp, "%s", "checked");
		else
			websWrite(wp, "%s", "");
	}
	else if (strcmp(argv[0], "3PTY_Conference")==0)
	{
		if(	pCfg->fxsport_flag & FXS_3WAY)
			websWrite(wp, "%s", "checked");
		else
			websWrite(wp, "%s", "");
	}
	...
	else if (strcmp(argv[0], "speed_dial_display") == 0)
	{
    	...
    	websWrite(wp, "<input type=\"radio\" name=\"digitmap_enable\" value=1 %s>On",
    				( pCfg ->digitmap_enable ? "checked" : "" ) );
    	websWrite(wp, "<input type=\"radio\" name=\"digitmap_enable\" value=0 %s>Off",
    				( !pCfg ->digitmap_enable ? "checked" : "" ) );
...
// ECM mode setting		
		...
		websWrite(wp, "%s", (pCfg->anstone_2&0x1) ? "checked" : "");
	else if (strcmp(argv[0], "useECM_PPSMPS_IP")==0)
		websWrite(wp, "%s", (pCfg->anstone_2&0x2) ? "checked" : "");
	else if (strcmp(argv[0], "useECM_PPSEOP_TDM")==0)
		websWrite(wp, "%s", (pCfg->anstone_2&0x4) ? "checked" : "");
	else if (strcmp(argv[0], "useECM_PPSEOP_IP")==0)
		websWrite(wp, "%s", (pCfg->anstone_2&0x8) ? "checked" : "");
	else if (strcmp(argv[0], "useECM_RNR_TDM")==0)
		websWrite(wp, "%s", (pCfg->anstone_2&0x10) ? "checked" : "");
	else if (strcmp(argv[0], "useECM_RNR_IP")==0)
		websWrite(wp, "%s", (pCfg->anstone_2&0x20) ? "checked" : "");
#endif
//
...
void asp_voip_GeneralSet(webs_t wp, char_t *path, char_t *query)
{
	...
	for (i=0; i< MAX_PROXY; i++)
	{
		...
		if (gstrcmp(websGetVar(wp, szName, ""), "on") == 0){
			//get vaule
			sprintf(szName, "sessionExpiry%d", i);
			pCfg->proxies[i].SessionUpdateTimer = atoi(websGetVar(wp, szName, "90"));
...
//	fprintf(stderr, "reject_direct_ip_call %d \n",atoi(websGetVar(wp, "reject_direct_ip_call", "")));
//	fprintf(stderr, "caller_id_hidden %d \n",atoi(websGetVar(wp, "caller_id_hidden", "")));
//	fprintf(stderr, "reject_direct_ip_call %s \n",websGetVar(wp, "reject_direct_ip_call", ""));
//	fprintf(stderr, "caller_id_hidden %s \n",websGetVar(wp, "caller_id_hidden", ""));
	
	if(!gstrcmp(websGetVar(wp, "reject_direct_ip_call", ""), "on")){
	//	fprintf(stderr, "reject direct ip on\n");
		pCfg->fxsport_flag |= FXS_REJECT_DIRECT_IP_CALL;
	}
	
	pCfg->fxsport_flag &= ~FXS_CALLERID_HIDDEN;
	if(!gstrcmp(websGetVar(wp, "caller_id_hidden", ""), "on")){
...
//	pCfg->direct_ip_call = gstrcmp(websGetVar(wp, T("reject_direct_ip_call"), T("")), "on");
    ...
    pCfg->fxsport_flag &= ~FXS_CALL_TRANSFER;
	if(!gstrcmp(websGetVar(wp, "Call_Transfer", ""), "on")){
//		fprintf(stderr, "Call_Transfer on\n");
		pCfg->fxsport_flag |= FXS_CALL_TRANSFER;
	}

	pCfg->fxsport_flag &= ~FXS_3WAY;
	if(!gstrcmp(websGetVar(wp, "3PTY_Conference", ""), "on")){
//		fprintf(stderr, "3PTY_Conference on\n");
		pCfg->fxsport_flag |= FXS_3WAY;
	}
	...
	pCfg ->digitmap_enable =  atoi( websGetVar(wp, "digitmap_enable", "") );
...
#if 0
	i = 0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_PPSMPS_TDM", ""), "on"))?0x1:0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_PPSMPS_IP", ""), "on"))?0x2:0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_PPSEOP_TDM", ""), "on"))?0x4:0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_PPSEOP_IP", ""), "on"))?0x8:0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_RNR_TDM", ""), "on"))?0x10:0;
	i|= (!gstrcmp(websGetVar(wp, "useECM_RNR_IP", ""), "on"))?0x20:0;	
