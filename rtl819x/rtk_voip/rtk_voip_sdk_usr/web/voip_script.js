...
function disablePrecedence(res_objs,index)
{
	var obj;
	obj = res_objs[index];
	...
	if(obj.checked == true)
	{
		res_objs[index].value = 100;
	}
	
}

function updatePrecedence_and_disable_check(chk_objs, res_objs,disable_obj)
{
	var len;
	var selection;
	...
	for (i=0; i<len; i++)
	{
		for (j=0; j<len; j++)
		{
			...
			{
				if(disable_obj[i].checked == true)
				{
					res_objs[i].value = 255;
				}
				else{
					res_objs[i].value = j;
					selection++;
...
function checkPrecedence_support_disable(objs,disable_obj, row, col, len, force_checked)
{
	var x, y;
	var i, len, idx;
	var obj;
...
function changeStartEnd() 
{
	...
	for (var i=0; i<sip_ports.length; i++)
	{
		...
		if (i == voipport) // the same page
		{
			...
			//for rostelecom t38 port need use same value with rtp. 
			//if (t38port && t38port >= min && t38port <= max)
			//{
...
//	}
	...
	if (updatePrecedence_and_disable_check(sipform.precedence, sipform.preced_id,sipform.codec_disable) == 0)
	{
