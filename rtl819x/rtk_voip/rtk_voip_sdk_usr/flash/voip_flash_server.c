...
int voip_share_get(voip_flash_share_t *pVoIPShare)
{
	printf("voip_share_get\n");
	if (sem_wait(VOIP_SEM_MUTEX) == -1)
		return -1;

	// update data from share memory 
	memcpy(pVoIPShare, g_shmVoIPCfg, sizeof(*g_shmVoIPCfg));
...
int voip_share_set(voip_flash_share_t *pVoIPShare)
{
	printf("voip_share_set\n");
	if (sem_wait(VOIP_SEM_MUTEX) == -1){
		fprintf(stderr, "VOIP_SEM_MUTEX Get failed: %s \n", strerror(errno));
		return -1;
	}
	//update voip config part.
	memcpy(&g_shmVoIPCfg->voip_cfg, &pVoIPShare->voip_cfg, sizeof(voipCfgParam_t));

	if (sem_signal(VOIP_SEM_MUTEX) == -1){
		fprintf(stderr, "VOIP_SEM_MUTEX Free Failed: %s\n", strerror(errno));
		return -1;
	}
...
int voip_flash_server_update()
{
	...
	else
	{
		...
		if(-1 == flash_voip_export_to_file(&VoIPallconfig, VOIP_CONFIG_PATH, 1))
			fprintf(stderr,"flash voip export to file fail\n");
	}
#endif
...
int voip_flash_server_init_variables()
{
	key_t key = ftok(VOIP_PATHNAME, VOIP_FLASH_VER);
	if (key == -1)
	{
		fprintf(stderr, "ftok failed: %s\n", strerror(errno));
		return -1;
	...
	// get share memory ID (voip flash)
	shm_id = shmget(key, sizeof(voip_flash_share_t), 0666 | IPC_CREAT);
	if (shm_id == -1)
	{
		fprintf(stderr, "shmget failed: %s\n", strerror(errno));
    ...
    // attach share memory (voip flash)
	if ((g_shmVoIPCfg = shmat(shm_id, NULL, 0)) == (void *)-1)
	{
		fprintf(stderr, "shmat failed: %s\n", strerror(errno));
		return -1;
	}    
