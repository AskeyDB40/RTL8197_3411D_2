...
voipCfgParam_t voipCfgParamDefault =
{
	...
	MAX_DIAL_NUMBER,		//dial number MAX length
	MIN_DIAL_NUMBER,		//dial number MIN length
	90,						//mini-SE
	// ports[VOIP_PORTS]
	{
		// port 0
		{
			// proxies
			{
				// proxies[0]
				{
					...
					"\0",				//RegistrarServer for tr104
					DEF_SIP_PROXY_PORT, // RegistrarServer port for tr104
					"\0",				// domain_name
					3600,				// reg_expire
			...
			DEF_MAX_RTP_PORT,	//MAX media_port
...
// refer to setFirewallIptablesRules()
int get_wan_interfaces(voip_flash_share_t *pVoIPShare)
{
	int opmode=-1;
	int wan_dhcp=-1;
	...
	// copy to net_cfg and return the number of interfaces
	strcpy(pVoIPShare->net_cfg.gateway_interface[0], pInterface);
	return 1;
}
#endif

//ericchung:  rcm_voip_flash_if_name to get default gateway interface name 
void rcm_voip_flash_if_name(voip_flash_share_t *pVoIPShare)
{
...
#endif
	unsigned char interface_name[DNS_LEN];
	int voip_index=0;
	...
	/*clean all interface list */
	for (i=0;i<MAX_VOIP_INTERFACE;i++){
		memset(pVoIPShare->net_cfg.gateway_interface[i],0,DNS_LEN);
	}

	/*search all wan interface */
#ifdef CONFIG_RTL_819X
	get_wan_interfaces(pVoIPShare);
	voip_if_index = 0;
#endif

 //Remove VoIP searching condition for considering dgw (default gateway) field as no VoIP traffic is desired if no IF is configured to it (recommended by SD5 Kaohj)
//	if(i<totalEntry)
	//	goto l_success_end;
#if 0 //Adam test
	totalEntry = mib_chain_total(MIB_ATM_VC_TBL);
	if(totalEntry == 0){
		printf("MIB_ATM_VC_TBL entry is empty\n");
	...
	for(i=0;i<totalEntry;i++){
		if(mib_chain_get(MIB_ATM_VC_TBL,i,&Entry)!=1)
			...
			ifGetName(Entry.ifIndex, pVoIPShare->net_cfg.gateway_interface[voip_index], DNS_LEN);
			voip_index++;
	...
	wan_3g_p=&wan_3g_Entry;
		snprintf( pVoIPShare->net_cfg.gateway_interface[voip_index],15, "ppp%d", MODEM_PPPIDX_FROM );
			printf("RCM: %s(%d) 3g   voip_index=%d voip_if_index=%d ifname=%s\n", 
			__FUNCTION__ , __LINE__ , voip_index , voip_if_index , pVoIPShare->net_cfg.gateway_interface[voip_index]);
		voip_if_index++;
		voip_index++;
#endif
#endif
	...
	/* it is hard copy, need modify it , if enable ZM8620, always add usb0 interface*/
	snprintf( pVoIPShare->net_cfg.gateway_interface[voip_index],15, "usb%d", 0 );
			printf("RCM: %s(%d) usb0 voip_index=%d voip_if_index=%d ifname=%s\n", 
			__FUNCTION__ , __LINE__ , voip_index , voip_if_index , pVoIPShare->net_cfg.gateway_interface[voip_index]);
		voip_if_index++;
		voip_index++;
	...
	if(voip_if_index == -1)
	{
		printf("VoIP:no wan interface find \n");
	}
...
int flash_voip_default(voipCfgParam_t *pVoIPCfg)
{
	...
	for (i=0; i<VOIP_PORTS; i++)
	{
		if (i > 0)
		{
			memcpy(&pVoIPCfg->ports[i], &pVoIPCfg->ports[i], sizeof(pVoIPCfg->ports[0]));
#if 0			
			pVoIPCfg->ports[i].sip_port = DEF_SIP_LOCAL_PORT + i;
#else
			pVoIPCfg->ports[i].sip_port = DEF_SIP_LOCAL_PORT; /* only use one port to receive SIP message  , exosip 3.5*/
#endif
		...
		{
			memcpy(&pVoIPCfg->ports[i].proxies[j], &pVoIPCfg->ports[i].proxies[j],
				sizeof(pVoIPCfg->ports[0].proxies[0]));
		}
	}
...
int flash_voip_check(voipCfgParam_t *pVoIPCfg)
{
	...
	if ( feature != feature_in_flash );
	{
		...
		if (!RTK_VOIP_CH_NUM_CMP(feature, feature_in_flash))
		{
			...
			{
				memcpy(&pVoIPCfg->ports[i], &voipCfgParamDefault.ports[i], sizeof(pVoIPCfg->ports[0]));
#if 0				
				pVoIPCfg->ports[i].sip_port = DEF_SIP_LOCAL_PORT + i;
#else
				pVoIPCfg->ports[i].sip_port = DEF_SIP_LOCAL_PORT;/*only one port for receive sip message , exosip 3.5 */ 
#endif
				pVoIPCfg->ports[i].media_port = DEF_RTP_PORT + i * 4;
				pVoIPCfg->ports[i].T38_port = DEF_T38_PORT + i * 2;
			}
	...
	if(mib_load(CURRENT_SETTING, CONFIG_MIB_CHAIN)){	//load flash to memory
		...
		if(0 == totalEntry){	//no entry in flash
			if(mib_chain_add(MIB_VOIP_CFG_TBL, (unsigned char*) &cfg_all->current_setting)){
				...
				fprintf(stderr, "fail to add voip config to memory 001 .\n");
				return -1;
			}
...
int voip_flash_get_mib_chain_total_entry(void)
{
	return mib_chain_total(MIB_VOIP_CFG_TBL);
}

int voip_shm_get_mib_chain_total_entry(void)
{
	return mib_chain_total(MIB_VOIP_CFG_TBL);
}

int voip_shm_get(voipCfgParam_t **ppVoIPCfg)
{
	return voip_flash_get(ppVoIPCfg);
}

int voip_shm_set(voipCfgParam_t *cfg)
{
	int ret=0;
	voipCfgAll_t cfg_all;
	...
	memcpy(&cfg_all.current_setting, cfg, sizeof(voipCfgParam_t));
	cfg_all.mode |= VOIP_CURRENT_SETTING;
	fprintf(stderr, "%s(%d) cfg_all.mode=%d\n" , __FUNCTION__ , __LINE__ , cfg_all.mode);
	ret = voip_shm_write(&cfg_all);
	
	if(ret){
		fprintf(stderr, "write voip shm failed.\n");
	}

	return ret;
}

int voip_shm_write(voipCfgAll_t *cfg_all)
{
	if (NULL == cfg_all){
		fprintf(stderr, "voip configure is NULL.\n");
		return -1;
	...
	if ((cfg_all->mode & VOIP_CURRENT_SETTING) == 0){
		fprintf(stderr, "%s(%d) NOK mode=%d\n" , __FUNCTION__ , __LINE__, cfg_all->mode);
		fprintf(stderr, "voip configure mode is NULL.\n");
		return -1;
	}
	else{
		fprintf(stderr, "%s(%d) OK mode=%d\n" , __FUNCTION__ , __LINE__, cfg_all->mode);
	}
	...
	mib_backup(CONFIG_MIB_CHAIN);	// backup current MIB in RAM
	if(mib_chain_update(MIB_VOIP_CFG_TBL, (void *) &cfg_all->current_setting, 0) == 0 ){
		fprintf(stderr, "update voip config to memory fail.\n");
		return -1;
	}
	mib_restore(CONFIG_MIB_CHAIN); //write back to memory
...
void rcm_voip_get_ctype_interface(voip_flash_share_t *pVoIPShare)
{
	...
	MIB_CE_ATM_VC_T Entry;
#endif
	fprintf(stderr, "rcm_voip_get_ctype_interface\n");
#if 0 //Adam test
	...
	//	ifGetName(Entry.ifIndex, interface_name, DNS_LEN);
		//get all voip interface 
		if (Entry.applicationtype&X_CT_SRV_VOICE){
			ifGetName(Entry.ifIndex, pVoIPShare->net_cfg.gateway_interface[voip_index], DNS_LEN);
			printf("voip_index %d ,interface name:%s\n",voip_index,pVoIPShare->net_cfg.gateway_interface[voip_index]);
			voip_index++;
			voip_if_index = Entry.ifIndex;
		}	
	}
...
//	fprintf(stderr, "Error: No valid IF for VoIP!!\n");
	return;
}
#endif
...
int voip_flash_server_read(voip_flash_share_t *pVoIPShare)
{
	...
	unsigned int totalEntry = 0;
	unsigned char interface_name[DNS_LEN];	
	unsigned char *interface=pVoIPShare->net_cfg.gateway_interface;

#ifdef CONFIG_USER_RTK_WAN_CTYPE
	rcm_voip_get_ctype_interface(pVoIPShare);
#else
	rcm_voip_flash_if_name(pVoIPShare); /*copy wan interface to pVoIPShare->net_cfg.gateway_interface */
#endif
	//printf("pVoIPShare->net_cfg.gateway_interface is %s\n",pVoIPShare->net_cfg.gateway_interface);
	...
	if( totalEntry > 0 ){
		...
		if(mib_chain_add(MIB_VOIP_CFG_TBL, (unsigned char*)&VoipEntry)){ //add to the voip setting to memory
			...
			fprintf(stderr, "fail to add voip config to memory 002.\n");
			return -1;
		}
		return -1;
	}
...
#if 0 //ericchung remove import/export voip mib to file.		
