 ...
 */
  typedef enum eXosip_event_type
  {
    ...
    /* INVITE related events within calls 5~ 15 */
 ...
 */
  struct eXosip_event
  {
    ...
    unsigned int server_expires;
#endif  

#ifdef ATTENDED_TRANSFER
    struct eXosip_dialog_t     *rd;	//the call/dialog to be replaced
    struct eXosip_call_t       *rc;
#endif
	//]
  };
 ...
 */
int eXosip_get_localip_for(char *host, char **localip);
#if 0//ericchung: temp mark , not need exosip 0.9 firewall api.
/**
 * This method is used to modify the sdp connection address
 * When you are behind a NAT. (EXPERIMENTAL)
 * 
 */
void rcm_eXosip_set_firewallip(const char *firewall_address, int nPort);


/* Mandy add for stun support */
void rcm_eXosip_set_firewallport(const int nPort, const char *firewall_port);
void rcm_eXosip_set_firewall_media_port(const int nPort, const int firewall_media_port);

/*++added by Jack Chan 05/03/07 for T.38++*/
void rcm_eXosip_set_faxport(const int nPort, const char *fax_port);
void rcm_eXosip_set_firewall_faxport(const int nPort, const int firewall_faxport);
/*--end--*/
...
/* Find the voip interface IP address */
int rcm_get_localip_for_voip(char *interfacename,char **localip);
