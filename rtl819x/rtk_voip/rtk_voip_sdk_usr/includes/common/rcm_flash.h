...
#define SIP_ANONYMOUSE_FROM				(1<<4)	/* when bit set 1, dial CLIR to sip server, from header use anonymouse */
...
#define SIP_DTMFMethodSIPInfoBody_TELEven	(1<<8)	/* when set 1, the format of the message body corresponds to the field Content-Type equal  audio / telephone-event. */

/*fxsport_flag define */
#define FXS_REJECT_DIRECT_IP_CALL	(1<<0)
#define FXS_CALLERID_HIDDEN			(1<<1)
#define FXS_CALL_TRANSFER			(1<<2)
#define FXS_3WAY					(1<<3)
...
#define URI_LEN					389		///< max URI length
...
#define	DEF_MAX_RTP_PORT		(DEF_RTP_PORT +10)
...
#define MAX_DIAL_NUMBER 40
#define MIN_DIAL_NUMBER 1
