...
struct CWMP_PRMT tVSCapSIPLeafInfo[] =
{
	...
	{ "EventSubscription",	eCWMP_tBOOLEAN,	CWMP_READ,	 &tVSCapSIPLeafOP},
	{ "ResponseMap",		eCWMP_tBOOLEAN,	CWMP_READ,	 &tVSCapSIPLeafOP},
};
enum eVSCapSIPLeaf
{
	eVSCapRole,
...
struct CWMP_LEAF tVSCapSIPLeaf[] =
{
    ...
    { &tVSCapSIPLeafInfo[eVSCapEventSubscription]  },
    { &tVSCapSIPLeafInfo[eVSCapResponseMap]  },
    { NULL }
};
...
struct CWMP_PRMT tVSCapabilitiesLeafInfo[] =
{
    ...
    { "MaxLineCount",      	  eCWMP_tUINT,    CWMP_READ,  &tVSCapabilitiesLeafOP},
    { "MaxSessionsPerLine",   eCWMP_tUINT,    CWMP_READ,  &tVSCapabilitiesLeafOP},
    { "RTCP",                 eCWMP_tBOOLEAN, CWMP_READ,  &tVSCapabilitiesLeafOP},
    { "NumberingPlan",        eCWMP_tBOOLEAN, CWMP_READ,  &tVSCapabilitiesLeafOP}
};
...
#endif    
    { &tVSCapabilitiesLeafInfo[eVSMaxLineCount]  },
    { &tVSCapabilitiesLeafInfo[eVSMaxSessionsPerLine]  },
	{ &tVSCapabilitiesLeafInfo[eVRTCP]  },
    { &tVSCapabilitiesLeafInfo[eVNumberingPlan]  },
...
int getCapabilitiesSipEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
    ...
    *data = NULL;
	...
	} else if( strcmp( pszLastname, "EventSubscription" ) == 0 ) {
		*data = booldup( TRUE );
	} else if( strcmp( pszLastname, "ResponseMap" ) == 0 ) {
		*data = booldup(TRUE);
	} else {
		*data = NULL;
		return ERR_9005;
...
int getCapabilitiesEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
    ...
    *data = NULL;
	...
	} else if( strcmp( pszLastname, "RTCP" ) == 0 ) {
		*data = booldup(TRUE);
	} else if( strcmp( pszLastname, "NumberingPlan" ) == 0 ) {
		*data = booldup(TRUE);
	} else if( strcmp( pszLastname, "MaxSessionsPerLine" ) == 0 ) {
		*data = uintdup( MAX_SESSION_COUNT );
	} else if( strcmp( pszLastname, "MaxLineCount" ) == 0 ) {
		*data = uintdup( MAX_PROFILE_COUNT );	
	} else {
		*data = NULL;
		return ERR_9005;
