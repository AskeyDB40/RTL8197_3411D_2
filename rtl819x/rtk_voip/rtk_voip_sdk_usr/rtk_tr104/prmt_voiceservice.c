...
int getVSEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
	...
	*data = NULL;
	...
	}else if( strcmp( lastname, "X_WANReferenceList" ) == 0 ) {
		*data = intdup( X_WANReferenceList );	
	} else{
		return ERR_9005;
	}
...
int setVSEntity(char *name, struct CWMP_LEAF *entity, int *type, void *data)
{
	 int res = 0;
	char *lastname = entity->info->name;
	...
	if( strcmp( lastname, "X_WANReferenceList" ) == 0 ) {
		 X_WANReferenceList	=*( ( int * )data ) ;
	} else {
		return ERR_9005;
	}
