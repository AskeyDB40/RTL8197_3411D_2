...
char PhyReferenceList[2][32];
int SIP_EthernetPriorityMark[2];
int RTP_EthernetPriorityMark[2];
...
//		if(proxy->enable && (0 == strlen((char *)pData) || NULL == pData)){
	...
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMIN:
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMAX:	
    ...
    case MIB_VOICE_PROFILE__RTP__DSCPMARK:
        ...
        break;
	case MIB_VOICE_PROFILE__RTP__TELEPHONEEVENTPAYLOADTYPE:
	        if(*( ( unsigned int * )pData ) < 0 || *( ( unsigned int * )pData ) > 128){
			CWMPDBG( 0, ( stderr, "ERROR:TelephoneEventPayloadType should be in range 0~128.\n") );
			return VERIFY_ERROR;
		}	
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MINIMUMNUMBEROFDIGITS:
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MAXIMUMNUMBEROFDIGITS:
		if(*( ( unsigned int * )pData ) < 1 || *( ( unsigned int * )pData ) > 40){
			CWMPDBG( 0, ( stderr, "ERROR:NUMBERINGPLAN should be in range 1~40.\n") );
			return VERIFY_ERROR;
		}	
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__INTERDIGITTIMERSTD:
		if(*( ( unsigned int * )pData ) < 1 || *( ( unsigned int * )pData ) > 5000){
				CWMPDBG( 0, ( stderr, "ERROR:INTERDIGITTIMERSTD should be in range 1~5000 and .\n") );
				return VERIFY_ERROR;
		}	
		break;
...
//#endif
	...
	case MIB_VOICE_PROFILE__DTMF_METHOD:
		*( ( DTMF_method_t * )pData ) = pVoipConfig ->ports[nPort].dtmf_mode;
		break;
	case MIB_VOICE_PROFILE__DTMF_METHODG711:
		*( ( DTMF_method_t * )pData ) = pVoipConfig ->ports[nPort].dtmf_mode;
		break;	
	case MIB_VOICE_PROFILE__DTMF_METHODSIPInfoBody:
		if (pVoipConfig->rfc_flags & SIP_DTMFMethodSIPInfoBody_TELEven)
		{
			strcpy( ( char * )pData, "TELEven");
		}
		else
		{
			strcpy( ( char * )pData, "DTMFrelay");
		}
		break;		
	...
	case MIB_VOICE_PROFILE__SIP__REGISTRAR_SERVER:
		strcpy( ( char * )pData, proxy1->RegistrarServer_addr);
		break;
		
	case MIB_VOICE_PROFILE__SIP__REGISTRAR_SERVER_PORT:
		*( ( unsigned int * )pData ) = proxy1->RegistrarServer_port;
		break;
   ...
   case MIB_VOICE_PROFILE__SIP__XCT_STB_REGISTERRETRYINTERVAL:
    ...
    case MIB_VOICE_PROFILE__SIP__REGISTEREXPIRES:
        *( ( unsigned int * )pData ) =  proxy1->reg_expire;
		break;
	case MIB_VOICE_PROFILE__SIP__REGISTERSMINEXPIRES:
		  *( ( unsigned int * )pData ) = pVoipConfig->MINI_SessionExpire;
		break;
	...
	case MIB_VOICE_PROFILE__SIP__XCT_STB_802_1PMARK:
		*( ( unsigned int * )pData ) = pVoipConfig->wanVlanPriorityData;		
		break;
	...
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMIN:
		//value 65535[0-65535]
			*( ( unsigned int * )pData ) = pVoipConfig->ports[nPort].media_port;
		break;
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMAX:
		//value 65535[0-65535]
			*( ( unsigned int * )pData ) = pVoipConfig->ports[nPort].media_port_max;
		break;
	case MIB_VOICE_PROFILE__RTP__TELEPHONEEVENTPAYLOADTYPE:
		//value 128[0-128]
			*( ( unsigned int * )pData ) = pVoipConfig->ports[nPort].dtmf_2833_pt;
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__XCT_NUMBERPLAN:
		...
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MINIMUMNUMBEROFDIGITS:
		//need implement:Temporary get max value 40[1-40]
		*( ( unsigned int * )pData ) = pVoipConfig->min_dial_num;
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MAXIMUMNUMBEROFDIGITS:
		//need implement:Temporary get max value 40[1-40]
		*( ( unsigned int * )pData ) =  pVoipConfig->max_dial_num;
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__INTERDIGITTIMERSTD:
		*( ( unsigned int * )pData ) = pVoipConfig ->auto_dial*1000;
		break;	
   ...
   case MIB_PHYINTERFACE__PHYPORT:
    ...
    case MIB_PHYINTERFACE__TESTS__TESTSTATE:
		if(pEvtMsg.e8c_autotest.TestStatus[nPort]==0)
			strcpy( ( char * )pData, "None");
		else if(pEvtMsg.e8c_autotest.TestStatus[nPort]==1)
    ...
    case MIB_PHYINTERFACE__TESTS__TESTSELECTOR:
		...
		if( pEvtMsg.e8c_autotest.TestSelector[nPort] == 0){
    ...
    case MIB_PHYINTERFACE__TESTS__PHONECONNECTIVITY:
		if (pEvtMsg.e8c_autotest.PhoneConnectivity[nPort]==0)
			*( ( boolean * )pData ) = FALSE;
		else
			*( ( boolean * )pData ) = TRUE;
		break;
    ...
    case MIB_PHYINTERFACE__TESTS__XCT_SIMULATETEST__FAILEDRESPONSECODE:
	      *( ( unsigned int * )pData ) = 1;
        break;
	case MIB_VOICE_PROFILE__RTCP__ENABLE:
		if ( pVoipConfig ->ports[nPort].RTCP_Interval==0){
			*( ( boolean* )pData )=0;
		}else{
			*( ( boolean* )pData )=1;
		}
		break;
	case MIB_VOICE_PROFILE__RTCP__TXREPEATINTERVAL:
		*( ( unsigned int * )pData ) = pVoipConfig ->ports[nPort].RTCP_Interval;
		break;
	case MIB_VOICE_PROFILE__FAXT38__ENABLE:
		  *( ( boolean* )pData )=pVoipConfig ->ports[nPort].useT38 ;
		break;
	case MIB_VOICE_PROFILE__FAX_PASSTHROUGH:
		
		strcpy( ( char * )pData, "Auto");
		break;			
	case MIB_VOICE_PROFILE__MODEM_PASSTHROUGH:
		
		strcpy( ( char * )pData, "Auto");
		break;
	case MIB_VOICE_PROFILE__SIP__EthernetPriorityMark:
		*( (  int * )pData ) = pVoipConfig->wanVlanPriorityData;		
		break;
	case MIB_VOICE_PROFILE__RTP__EthernetPriorityMark:
		*( (  int * )pData ) = pVoipConfig->wanVlanPriorityVoice;		
		break;
	default:
...
int mib_set_type1( idMib_t idMib, unsigned int nInstNum1, void *pData )
{	
	/* VoiceService.1.VoiceProfile.{ i }. */
...
//#endif
	...
	case MIB_VOICE_PROFILE__DTMF_METHODG711:
		...
		break;
	case MIB_VOICE_PROFILE__DTMF_METHODSIPInfoBody:
		if(strcmp(( char * )pData,"TELEven")==0){
			pVoipConfig->rfc_flags |= SIP_DTMFMethodSIPInfoBody_TELEven;
			bDirtyBit = TRUE;
		}
		else if(strcmp(( char * )pData,"DTMFrelay")==0){
			pVoipConfig->rfc_flags &= ~SIP_DTMFMethodSIPInfoBody_TELEven;
			bDirtyBit = TRUE;
		}
		break;		
	...
	case MIB_VOICE_PROFILE__SIP__REGISTRAR_SERVER:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			strcpy( proxy1->RegistrarServer_addr, ( char * )pData );
			bDirtyBit = TRUE;
		}
		break;
	...
	case MIB_VOICE_PROFILE__SIP__REGISTRAR_SERVER_PORT:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			proxy1->RegistrarServer_port = *( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		}
		break;
    ...
    case MIB_VOICE_PROFILE__SIP__REGISTERSMINEXPIRES:
            /* main and backup proxy use the same value */
			pVoipConfig->MINI_SessionExpire = *( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		break;		
	...
	case MIB_VOICE_PROFILE__RTP__XCT_STB_802_1PMARK:	
		pVoipConfig ->wanVlanPriorityData =  *( ( int * )pData );
		bDirtyBit = TRUE;
		break;
	case MIB_VOICE_PROFILE__SIP__XCT_STB_802_1PMARK:
		/* Need check */
...
//			proxy2->enable|=PROXY_OPTIONS_PING;
	...
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMIN:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig->ports[nPort].media_port=*( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		}
		break;
	case MIB_VOICE_PROFILE__RTP__LOCALPORTMAX:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig->ports[nPort].media_port_max=*( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		}
		break;
	case MIB_VOICE_PROFILE__RTP__TELEPHONEEVENTPAYLOADTYPE:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig->ports[nPort].dtmf_2833_pt= *( ( unsigned int * * )pData );
			bDirtyBit = TRUE;
		}
		break;
	case MIB_VOICE_PROFILE__RTCP__ENABLE:
		if(*( ( boolean * )pData )==0){
			pVoipConfig ->ports[nPort].RTCP_Interval=0;
		}
		else{
			pVoipConfig ->ports[nPort].RTCP_Interval=10;
		}
		break;
	case MIB_VOICE_PROFILE__RTCP__TXREPEATINTERVAL:
		pVoipConfig ->ports[nPort].RTCP_Interval=*( ( unsigned int * * )pData );
		bDirtyBit = TRUE;
		break;
    ...
    case MIB_PHYINTERFACE__TESTS__TESTSTATE:
		...
		if(strcmp(( char * )pData ,"Complete")==0){
			pEvtMsg.e8c_autotest.TestStatus[nPort]=2;
		}else if (strcmp(( char * )pData ,"Requested")==0){
			pEvtMsg.e8c_autotest.TestStatus[nPort]=1;
			pEvtMsg.e8c_autotest.enable=1;
		}else{
			pEvtMsg.e8c_autotest.TestStatus[nPort]=0; /*None*/
		}
		break;
    ...
    case MIB_PHYINTERFACE__TESTS__TESTSELECTOR:
		...
		if(strcmp(( char * )pData ,"PhoneConnectivityTest")==0){
			pEvtMsg.e8c_autotest.TestSelector[nPort]=0;
		}else if (strcmp(( char * )pData ,"X_SimulateTest")==0){
			pEvtMsg.e8c_autotest.TestSelector[nPort]=1;
		}
		break;
	...
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MINIMUMNUMBEROFDIGITS:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig->min_dial_num=*( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		}
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__MAXIMUMNUMBEROFDIGITS:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig->max_dial_num=*( ( unsigned int * )pData );
			bDirtyBit = TRUE;
		}
		break;
	case MIB_VOICE_PROFILE__NUMBERINGPLAN__INTERDIGITTIMERSTD:
		if(VERIFY_OK == verifySettingValues(idMib, nPort, pVoipConfig, pData)){
			pVoipConfig ->auto_dial =  (*( ( unsigned int * )pData )/1000);
			bDirtyBit = TRUE;
		}
		break;
    case MIB_VOICE_PROFILE__NUMBERINGPLAN__XCT_NUMBERPLAN:
	...
	case MIB_VOICE_PROFILE__FAX_PASSTHROUGH:
		if(strcmp(( char * )pData,"Disable")==0){
			pVoipConfig ->ports[nPort].useT38=1;
		}else if(strcmp(( char * )pData,"Auto")==0){
			//not change
		}
		else if(strcmp(( char * )pData,"Force")==0){
			pVoipConfig ->ports[nPort].useT38=0;
		}
		break;
	case MIB_VOICE_PROFILE__MODEM_PASSTHROUGH:
		if(strcmp(( char * )pData,"Disable")==0){
...
int mib_get_type2( idMib_t idMib, unsigned int nInstNum1, unsigned int nInstNum2, void *pData )
{
...
//		*( ( line_status_t * )pData ) = UP;
	...
	case MIB_VOICE_PROFILE__LINE__CALLSTATUS:
		//need implement: Temporarily get vale idle 
		*( ( line_callstatus_t * )pData ) =0;
		break;
...
//		strcpy( (char *)pData, proxy2->password);
	...
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_CALLERIDENABLE:

		if(pVoipConfig ->ports[nPort].fxsport_flag & FXS_CALLERID_HIDDEN)
			*( ( unsigned int * )pData )=0;
		else
			*( ( unsigned int * )pData )=1;		
		break;

	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_CallWaitingStatus:

	/*from 104 spec.
	Status of endpoint managed call waiting, if
	supported. Enumeration of:
	¡§Disabled¡¨
	¡§Idle¡¨
	¡§SecondaryRinging¡¨
	¡§SecondaryConnecting¡¨
	...
	*/
	//ericchung TBD:
		strcpy( (char *)pData, "Idle");
		break;

	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_MaxSessions:		
			*( ( unsigned int * )pData )=2; //always 2 session.
		break;
		
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_ConferenceCallingStatus:		
/*  from 104 spec
			¡§Disabled¡¨
			¡§Idle¡¨
			¡§SecondaryCalling¡¨
...
*/
	...
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_CallTransferEnable:

		if(pVoipConfig ->ports[nPort].fxsport_flag & FXS_CALL_TRANSFER)
			*( ( unsigned int * )pData )=1; //enable
		else
			*( ( unsigned int * )pData )=0; //disable			
		break;

	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_3PTYconferenceEnable:

		if(pVoipConfig ->ports[nPort].fxsport_flag & FXS_3WAY)
			*( ( unsigned int * )pData )=1; //enable
		else
			*( ( unsigned int * )pData )=0; //disable			
		break;
	case MIB_VOICE_PROFILE__LINE__PhyReferenceList:
		strcpy( (char *)pData, PhyReferenceList[nPort]);
		break;
	default:
		return 0;	/* range error */
		break;
...
int mib_set_type2( idMib_t idMib, unsigned int nInstNum1, unsigned int nInstNum2, void *pData )
{
	/* VoiceService.1.VoiceProfile.{ i }.Line.{i}. */
	//voipCfgParam_t voipConfig;
	//voipCfgParam_t * const pVoipConfig = &g_pVoIPShare ->voip_cfg;
...
//#endif
	...
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_CALLERIDENABLE:
		if(*( ( enable_t * )pData ) == TR104_DISABLE){
			pVoipConfig ->ports[nPort].fxsport_flag |= FXS_CALLERID_HIDDEN;		
		}else{
			pVoipConfig ->ports[nPort].fxsport_flag &= ~FXS_CALLERID_HIDDEN;
		}
		bDirtyBit = TRUE;
	...
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_CallTransferEnable:
		...
		if(*( ( enable_t * )pData ) == TR104_DISABLE){
			pVoipConfig ->ports[nPort].fxsport_flag &= ~FXS_CALL_TRANSFER;		
		}else{
			pVoipConfig ->ports[nPort].fxsport_flag |= FXS_CALL_TRANSFER;
		}
		bDirtyBit = TRUE;
	...
	case MIB_VOICE_PROFILE__LINE__CALLING_FEATURES_3PTYconferenceEnable:
		...
		if(*( ( enable_t * )pData ) == TR104_DISABLE){
			pVoipConfig ->ports[nPort].fxsport_flag &= ~FXS_3WAY;		
		}else{
			pVoipConfig ->ports[nPort].fxsport_flag |= FXS_3WAY;
		}
		bDirtyBit = TRUE;
	...
	case MIB_VOICE_PROFILE__LINE__PhyReferenceList:
		strcpy( PhyReferenceList[nPort],(char *)pData);
		bDirtyBit = TRUE;
		break;	
...
int mib_get_type3( idMib_t idMib, unsigned int nInstNum1, unsigned int nInstNum2, unsigned int nInstNum3, void *pData )
{
...
//		strcpy( pData, lstCodecs[nInstNum3].pszPacketizationPeriod );
	...
	case MIB_VOICE_PROFILE__LINE__SIP__CODEC__LIST__ENABLE:
		//need implement 
		(*( boolean*)pData) =0;
		break;

	case MIB_VOICE_PROFILE__LINE__SESSION__SESSIONDURATION:
		*( ( unsigned int * )pData ) = 1;
		break;

	case MIB_VOICE_PROFILE__LINE__SESSION__FARENDIPADDRESS:
		strcpy( ( char * )pData, "192.168.1.0");
		break;

	case MIB_VOICE_PROFILE__LINE__SESSION__FARENDUDPPORT:
		*( ( unsigned int * )pData ) = 1;
		break;

	case MIB_VOICE_PROFILE__LINE__SESSION__LOCALUDPPORT:
		*( ( unsigned int * )pData ) = 1;
		break;
...
int mib_set_type3( idMib_t idMib, unsigned int nInstNum1, unsigned int nInstNum2, unsigned int nInstNum3, void *pData )
{
	/* VoiceService.1.VoiceProfile.{ i }.Line.{i}.Codec.List.{i}. */
	//voipCfgParam_t voipConfig;
	//voipCfgParam_t * const pVoipConfig = &g_pVoIPShare ->voip_cfg;
