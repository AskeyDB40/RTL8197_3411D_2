...
//******* VoiceService.{i}.PhyInterface.{i}.Tests.X_RTK_LineTest. EntityLEAF*******
struct CWMP_OP tTestsX_RTK_LineTestEntityLeafOP = { getX_RTK_LineTestEntity, setX_RTK_LineTestEntity};	

struct CWMP_PRMT tTestsX_RTK_LineTestEntityLeafInfo[] =
{
    /*(name,                   type,           flag,                 op)*/
    { "Conclusion",		eCWMP_tSTRING,  	CWMP_READ,				&tTestsX_RTK_LineTestEntityLeafOP},
    { "IsTestOnBusy",	eCWMP_tUINT,  	CWMP_READ|CWMP_WRITE,	&tTestsX_RTK_LineTestEntityLeafOP},

};

enum tTestsX_RTK_LineTestEntityLeaf
{
    eX_RTK_LT_Conclusion,
    eX_RTK_LT_IsTestOnBusy,
};

struct CWMP_LEAF tTestsX_RTK_LineTestEntityLeaf[] =
{
	{ &tTestsX_RTK_LineTestEntityLeafInfo[eX_RTK_LT_Conclusion]  },
	{ &tTestsX_RTK_LineTestEntityLeafInfo[eX_RTK_LT_IsTestOnBusy]  },
    { NULL }
};
//******* VoiceService.{i}.PhyInterface.{i}.Tests.X_RTK_InnerTest. EntityLEAF*******
struct CWMP_OP tTestsX_RTK_InnerTestEntityLeafOP = { getX_RTK_InnerTestEntity,NULL};	

struct CWMP_PRMT tTestsX_RTK_InnerTestEntityLeafInfo[] =
{
    /*(name,                   type,           flag,                 op)*/
    { "OnOffHookTestResult",              eCWMP_tSTRING,  CWMP_READ, &tTestsX_RTK_InnerTestEntityLeafOP},

};

enum tTestsX_RTK_InnerTestEntityLeaf
{
    eX_RTK_InnerTest_OnOffHookTestResult,
};


struct CWMP_LEAF tTestsX_RTK_InnerTestEntityLeaf[] =
{
	{ &tTestsX_RTK_InnerTestEntityLeafInfo[eX_RTK_InnerTest_OnOffHookTestResult]  },
    { NULL }
};
...
{
    /*(name,                  type,           flag,       op)*/
    {"X_RTK_LineTest",  eCWMP_tOBJECT,  CWMP_READ,  NULL},
    {"X_RTK_InnerTest", eCWMP_tOBJECT,  CWMP_READ,  NULL},
    {"X_SimulateTest",  eCWMP_tOBJECT,  CWMP_READ,  NULL},

};
enum ePhyInfTestsEntityObject
{
	eTests_X_RTK_LineTest,
	eTests_X_RTK_InnerTest,
...
struct CWMP_NODE tPhyInfTestsEntityObject[] =
{
	...
	{&tPPhyInfTestsEntityObjectInfo[eTests_X_RTK_LineTest],  tTestsX_RTK_LineTestEntityLeaf,  NULL},
	{&tPPhyInfTestsEntityObjectInfo[eTests_X_RTK_InnerTest], tTestsX_RTK_InnerTestEntityLeaf, NULL},
#endif	
...
int getX_RTK_LineTestEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
	/* VoiceService.{i}.PhyInterface.{i}.Tests.X_RTK_LineTest. */
	unsigned int nPhyInterfaceInstNum;
	union {
        char Conclusion[16];
        unsigned int  IsTestOnBusy;
	} s;
	...
	*data = NULL;
	...
	if( strcmp( pszLastname, "Conclusion" ) == 0 ) {
		*data = strdup("Normal");
	} else if( strcmp( pszLastname, "IsTestOnBusy" ) == 0 ) {
		*data = uintdup(0);
	} else {
		return ERR_9005;
	}
...
int setX_RTK_LineTestEntity(char *name, struct CWMP_LEAF *entity, int type, void *data)
{
	/* VoiceService.{i}.PhyInterface.{i}.Tests.X_RTK_LineTest. */
    int res = 0;
	unsigned int nPhyInterfaceInstNum;
...
int getX_RTK_InnerTestEntity(char *name, struct CWMP_LEAF *entity, int *type, void **data)
{
	/* VoiceService.{i}.PhyInterface.{i}.Tests.X_RTK_LineTest. */
	unsigned int nPhyInterfaceInstNum;
	...
	*data = NULL;
	...
	if( strcmp( pszLastname, "OnOffHookTestResult" ) == 0 ) {

		stVoipCfg.ch_id = nPhyInterfaceInstNum;
		rtk_GetPhoneState(&stVoipCfg);
		//printf("chid =%d GetHookStatus: %ld\n", nPhyInterfaceInstNum, stVoipCfg.cfg);

		// (0: on-hook, 1: off-hook) 
		if (stVoipCfg.cfg==1)
			*data = strdup("PhoneOffhook");
		else
			*data = strdup("PhoneOnhook");
	} else {
		return ERR_9005;
	}
