...
{
	printf(
		"usage:	%s DTMF <caller_id> <DTMF mode> <DTMF duration> <DTMF interdigit pause duration> \n" \
		"       %s FSK <caller_id> <FSK area> \n" \
		"       %s AUTO_test <test_set>\n"	\
		"  - caller_id => caller id string\n" \
		
		"  - DTMF mode[bit7]==> Auto SLIC action, 0: Disable, - 1: Enable.\n"
 		"  - DTMF mode[bit6]=> Before 1st Ring, 0: after 1st Ring, 1: before 1st Ring.\n"
		"  - DTMF mode[bit5]=> Auto Ring, 0: disable, 1: enable\n"	\
		"     SLIC action, such as SLIC hook statue check, line polarity change (not include SLIC Ringing)\n"
		"  - DTMF mode[bit4]=> Auto start/end digit send, 0:non-auto mode 1:auto mode\n" \
		...
		"  - DTMF mode[bit2-3]=> End digit, 0:A, 1:B, 2:C, 3:D\n"	\
		"  - DTMF mode[bit0-1]=> Start digit, 0:A, 1:B, 2:C, 3:D\n" \
		...
		"  - FSK area[bit0-2]=> 0 BELLCORE, 1: ETSI, 2: BT, 3: NTT\n" \
		"  - test_set => 1:test items are auto ring, and auto SLIC action\n"
 		"                2:test items are NO auto ring, and NO auto SLIC action\n"
		"                 3:run all sample\n\n\n"
 		"Example of DTMF mode ring + 'A' + '0123456789' + 'C' + ring\n"\
		"  caller_id DTMF 0123456789 184 80 80\n"	\
 		"Example of FSK mode '0123456789', area ETSI, auto SLIC action\n"	\
		"  caller_id FSK 0123456789 257 \n"	\
 		"Example of DTMF auto test all case\n"\
		"  caller_id AUTO_test 3\n" \
		, cmd, cmd, cmd);
	exit(0);
}
...
{
	unsigned int  dtmf_mode, dtmf_mode2, digit_on, digit_pause, fsk_area;
	...
	case 'D':
		...
		//=====config DTMF 
		rtk_SetDtmfCidParam(0, dtmf_start, dtmf_end, dtmf_mode2, digit_on, digit_pause, 300, 300); //simple implement: pre_silence = 300ms, end_silence = 300ms

		//=====gen DTMF CID
		// check auto rung and first ring before CID
	...
	case 'F':
		if (argc == 4)
		{
			...
			{	
				...
				rtk_GenFskCid(0, argv[2],  (void *) 0,  (void *) 0, 0/*FSK Type 1*/); // 
				//rtk_GenFskCid( 0, "9876543210", (void *) 0, (void *) 0, 0);
