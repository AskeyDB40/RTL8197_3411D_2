...
int main(int argc , char *argv[])
{
	...
	mode = atoi( argv[1] );   //0 polling. 1 is select.
	printf("Fxo_dual support polling and select mode  to get slic event\n");
	printf("current mode:");
	if( mode == 0 )
		printf("polling mode\n");
	else if( mode == 1 )
	...
	for (chid=0; chid<CON_CH_NUM; chid++)
	{
		...
		//rtk_SetEchoTail(chid, 64, 1, 5, 5, 0);			// 4ms, NLP on
		...
		if (CHANNEL_IS_FXS(chid))
		{
			...
			//rtk_SetSlicRingCadence(chid, 1500, 1500);	// 1500 ms
	...
	if( mode==1 ){  // select mode
		...
		for ( chid = 0 ; chid <CON_CH_NUM ; chid ++ )
		{
			...
			for ( i = 0 ; ; i ++ )
			{
				...
				//printf( "%s(%d) Event=0x%08x\n" , __FUNCTION__ , __LINE__ , stVoipEvent.id);
...
int do_fax_event( int chid, VoipEventID evt )
{
	...
	//printf("%s(%d) ch=%d fax=%u fax_clear=%d\n", __FUNCTION__ , __LINE__ , chid , channel->fax , channel->fax_clear);
	// Thlin add: once detect CED, change codec to T.38
	/*************** FAX Det ****************/
	if (CHANNEL_IS_FXS(chid))
	{
		//rtk_GetFaxModemEvent(chid, &channel->fax, NULL, 0);
		
		//1:low-speed fax, 2: high-speed fax, 3:local-modem, 4: remote-modem
		//if (((channel->fax == 1) || (channel->fax == 2))	&& 
		//	(channel->fax_clear == 1) )
		if (channel->fax_clear == 1)
		{
			//printf("Get FAX event, ch=%d\n", chid);
			printf("Get FAX event, ch=%d fax=%u fax_clear=%d\n", chid , channel->fax , channel->fax_clear);
			#ifdef FAX_USE_T38
	...
	if ( evt == VEID_FAXMDM_FAX_DIS_TX )
	{
		printf("Get FAX DIS TX event, ch=%d\n", chid);
	}
	else if ( evt == VEID_FAXMDM_FAX_DIS_RX ) 
	{
	...
	if ((channel->fax_end == 1) && (channel->fax_end_clear == 1))
	{
		printf("%s(%d) Get FAX End event, ch=%d\n", __FUNCTION__ , __LINE__ ,chid);
		channel->fax_end_clear = 0;
	}

	if ( evt == VEID_FAXMDM_T38_FAX_END )
	{
		printf("%s(%d) VEID_FAXMDM_T38_FAX_END ch=%d\n", __FUNCTION__ , __LINE__ ,chid);
	}
