...
int32 rtk_GetFxsEvent(uint32 chid, SIGNSTATE *pval)
{
	...
	*pval = SIGN_NONE;
	...
	if ( SlicEvent == SLICEVENT_OFFHOOK_2 ) /* PHONE_STILL_OFF_HOOK */
	{
		...
		if (Event == 'E' )
			printf("%s: dir=0, energey=%ddBFS, duration=%dms\n", __func__, Energy, Duration);
...
int32 rtk_SetNeonLampState(uint32 chid, TstFskClid* pVmwi)
{
    if( pVmwi==NULL ){
        TAPI_DUMP( "chid=%u, pVmwi is NULL\n", chid);
        return -1;
    }

    SETSOCKOPT(VOIP_MGR_SLIC_NEON_LAMP, pVmwi, TstFskClid, 1);
