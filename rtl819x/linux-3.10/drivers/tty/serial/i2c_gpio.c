...
{
		//rtl8196e_init_reg_gpio(); //move to always do it when read/write 
    return 0;
}
...
static int __init mfi_ioctl_init(void)
{
	...
	printk("Realtek GPIO to I2C Driver Init...\n");

    //rtl8196e_init_procfs();  //old code, remove it 
	...
	printk("%s:%d,dev=%d,MINOR=%d\n",__func__,__LINE__,MAJOR(dev),MINOR(dev));
    return 0;
}
...
{
    //cleanup_procfs_i2cG();  //remove it , old code
...
#define RTL_R32(addr)           (*(volatile unsigned long *)(addr))
	RTL96E_BOND=RTL_R32(RTL96E_BONDING);
	if(RTL96E_BOND == EU_VE3){
		...
		//printk("(RTL96E_BOND == E_VE3");
		
	}
#elif defined(CONFIG_RTL_8871AM) || defined(CONFIG_RTL_8881A_SELECTIVE)
        ...
        //RTL_W32(PIN_MUX_SEL_2, (REG32(PIN_MUX_SEL_2) | 0x603));//SET SHARED PIN
		RTL_W32(GPABCDCNR, (REG32(GPABCDCNR) & (~(0x3000000))));//SET GPIO FUNCTION
		MFi3959_i2c_dev.sdio = I2C_GPIO_ID( 'D', 1 );
		MFi3959_i2c_dev.sclk = I2C_GPIO_ID( 'D', 0 );
#elif defined(CONFIG_RTL_8197F)
        RTL_W32(GPABCDCNR, (REG32(GPABCDCNR) & (~(0x8000000))));//SET GPIO FUNCTION, D7
		RTL_W32(GPEFGHCNR, (REG32(GPEFGHCNR) & (~(0x0000001))));//SET GPIO FUNCTION, E0
		MFi3959_i2c_dev.sdio = I2C_GPIO_ID( 'E', 0 );
		MFi3959_i2c_dev.sclk = I2C_GPIO_ID( 'D', 7 );
#endif	
...
#if 1//(LINUX_VERSION_CODE == KERNEL_VERSION(3,10,24))
ssize_t i2c_proc_read(struct file *file, char __user *buf, size_t size, loff_t *pos) {
	return proc_read_i2cG(NULL, NULL, 0, 0, NULL, NULL);
}

ssize_t i2c_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *pos) {
	return proc_write_i2cG(file, buf, size, &i2cG_data);
}

struct file_operations i2c_proc_fops = {
	.read = i2c_proc_read,
	.write = i2c_proc_write,
};
#endif
...
//#elif (LINUX_VERSION_CODE == KERNEL_VERSION(3,10,24))

	//printk("proc_create\n");
	i2cGProcEntry = proc_create("i2cG", S_IRUGO, NULL, &i2c_proc_fops );
	//i2cGProcEntry = proc_create_data("i2cG", S_IRUGO, NULL, &i2c_proc_fops, NULL);
#endif
	if(i2cGProcEntry == NULL) {
		ret = -ENOMEM;
		printk("Create i2cG error!\n");
		goto no_i2cG;
	}
