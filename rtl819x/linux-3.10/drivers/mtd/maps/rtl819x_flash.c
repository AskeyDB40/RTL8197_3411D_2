...
#ifdef CONFIG_APPLE_HOMEKIT_BLOCK_SUPPORT
#define RTL_HOMEKIT_SIZE CONFIG_APPLE_HOMEKIT_BLOCK_SIZE
#define RTL_802_1X_CLIENT_SIZE 0
#define RTL_WAPI_SIZE 0
#else
...
static struct mtd_partition rtl8196_parts1[] = {
		...
		,
		{
			...
			offset: (CONFIG_RTL_FLATFS_IMAGE_OFFSET - \
					 RTL_HOMEKIT_SIZE),
#else
		       // squash mtd block homekit information here
			offset: (CONFIG_RTL_FLASH_SIZE - \
					 RTL_HOMEKIT_SIZE),
#endif
#endif		
...
static struct mtd_partition rtl8196_parts1[] = {
		...
		,
		{
			...
			size:	(RTL_802_1X_CLIENT_SIZE - 0),
			offset:	(CONFIG_RTL_FLASH_SIZE - \
					 RTL_802_1X_CLIENT_SIZE - RTL_WAPI_SIZE ),
		}
#endif
#ifdef CONFIG_RTL_WAPI_SUPPORT		
		,
...
#else
#ifdef CONFIG_MTD_CONCAT
	mtd_device_parse_register(mymtd, probes, NULL, NULL, 0);
#else
	ppdata.origin = 0;
	mtd_device_parse_register(my_sub_mtd[0], probes, &ppdata, NULL, 0);
	#if defined(CONFIG_RTL_TWO_SPI_FLASH_ENABLE)
		ppdata.origin = 1;
		mtd_device_parse_register(my_sub_mtd[1], probes, &ppdata, NULL, 0);
	#endif
#endif
#endif
