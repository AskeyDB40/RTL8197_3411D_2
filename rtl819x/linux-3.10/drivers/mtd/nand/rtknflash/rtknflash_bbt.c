...
static int rtk_scan_read_oob(struct mtd_info *mtd, uint8_t *buf, int  page,
			 size_t len)
{
	int res;
			  ...
			  uint8_t *buf,uint8_t *oob)
{
	...
	return rtkn_ecc_write_page0(mtd,this,buf,1);
}
...
static int rtk_scan_erase_bbt(struct mtd_info *mtd, int  page)
{	
	struct nand_chip* this = (struct nand_chip*)mtd->priv;
	struct rtknflash* rtkn = (struct rtknflash*)this->priv;
	rtkn->curr_page_addr = page;
...
static int rtk_block_isbad(struct mtd_info *mtd, u16 chipnr, loff_t ofs)
{
	...
	if ( isLastPage ){
		...
		if(rtk_scan_read_oob(mtd,rtkn->buf,page,page_size)){
...
static int rtk_create_bbt(struct mtd_info *mtd, int page)
{
	...
	//this->select_chip(mtd, 0);
  	...
  	if(rtk_scan_erase_bbt(mtd,page)){
		...
		//mark as bad block;
		if (isLastPage){
			rtk_scan_write_bbt(mtd,(page+ppb-1),page_size,temp_BBT,&rtkn->buf[page_size]);
			rtk_scan_write_bbt(mtd,(page+ppb-2),page_size,temp_BBT,&rtkn->buf[page_size]);
		}else{
			rtk_scan_write_bbt(mtd,(page),page_size,temp_BBT,&rtkn->buf[page_size]);
			rtk_scan_write_bbt(mtd,(page+1),page_size,temp_BBT,&rtkn->buf[page_size]);
		}		
		goto EXIT;
	}
	...
	while( mem_page_num>0 ){
		if(rtk_scan_write_bbt(mtd,(page+page_counter),page_size,temp_BBT+page_counter*page_size,&rtkn->buf[page_size])){
...
static int rtk_nand_scan_bbt(struct mtd_info *mtd)
{
	...
	for(i=0;i<BACKUP_BBT;i++){
		rc = rtk_scan_read_oob(mtd,rtkn->buf,(bbt_page+(i*ppb)),page_size);
		#ifdef SWAP_2K_DATA
	        ...
	        if(!rc){
			    ...
			    //if(0){
			        ...
			        while( mem_page_num_tmp>0 ){
			        	#if 1
				        if(rtk_scan_read_oob(mtd,rtkn->buf,(bbt_page+(i*ppb)+page_counter),page_size)){
							...
							if ( isLastPage){
								rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+(ppb-1)),page_size,temp_BBT,&rtkn->buf[page_size]);
								rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+(ppb-2)),page_size,temp_BBT,&rtkn->buf[page_size]);
							}else{
								rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)),page_size,temp_BBT,&rtkn->buf[page_size]);
								rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+1),page_size,temp_BBT,&rtkn->buf[page_size]);
							}					        
                            load_bbt_error=1;
			                error_count++;					        
					        break;
						...
						if(isbbt == BBT_TAG){//check bb tag in each page!
							memcpy( temp_BBT+page_counter*page_size, rtkn->buf, page_size );				        
				        	page_counter++;
				        	mem_page_num_tmp--;
						}else{
                            load_bbt_error=1;
...
static int create_v2r_remapping(struct mtd_info *mtd, unsigned int page, unsigned int block_v2r_num)
{
	...
	unsigned int offs=0, offs_real=0,ppb;
	...
	if(rtk_scan_erase_bbt(mtd,page)){
		...
		//mark as bad block;
		if ( isLastPage){	
			rtk_scan_write_bbt(mtd,(page+ppb-1),page_size,rtkn->buf,&rtkn->buf[page_size]);
			rtk_scan_write_bbt(mtd,(page+ppb-2),page_size,rtkn->buf,&rtkn->buf[page_size]);
		}else{
			rtk_scan_write_bbt(mtd,(page),page_size,rtkn->buf,&rtkn->buf[page_size]);
			rtk_scan_write_bbt(mtd,(page+1),page_size,rtkn->buf,&rtkn->buf[page_size]);
		}	
		goto EXIT_V2R;
	}
...
static int rtk_scan_v2r_bbt(struct mtd_info *mtd)
{
	...
	for(i=0;i<BACKUP_BBT;i++){
		nand_printf("[%s]:%d\n",__func__,__LINE__);
		rc = rtk_scan_read_oob(mtd,rtkn->buf,(bbt_v2r_page+(i*ppb)),page_size);
		nand_printf("[%s]:%d\n",__func__,__LINE__);
#ifdef SWAP_2K_DATA
		isbbt = rtkn->buf[page_size+OOB_BBI_OFF];
#else
	        ...
	        if(!rc){
			    ...
			    //if(0){
			        ...
			        while( mem_page_num_tmp>0 ){
				        ...
				        if( rtk_scan_read_oob(mtd, rtkn->buf, (bbt_v2r_page+(i*ppb)+page_counter),page_size) ){	
							...
							if ( isLastPage){
								rtk_scan_write_bbt(mtd,(bbt_v2r_page+(ppb*i)+(ppb-1)),page_size,temp_BBT,&rtkn->buf[page_size]);
								rtk_scan_write_bbt(mtd,(bbt_v2r_page+(ppb*i)+(ppb-2)),page_size,temp_BBT,&rtkn->buf[page_size]);
							}else{
								rtk_scan_write_bbt(mtd,(bbt_v2r_page+(ppb*i)),page_size,temp_BBT,&rtkn->buf[page_size]);
								rtk_scan_write_bbt(mtd,(bbt_v2r_page+(ppb*i)+1),page_size,temp_BBT,&rtkn->buf[page_size]);
							}					        
                            load_bbt_error=1;
			                error_count++;					        
					        break;
...
static int rtk_update_bbt (struct mtd_info *mtd, BB_t *bbt)
{
    ...
    for(i=0;i<BACKUP_BBT;i++){
		...
		while( mem_page_num_tmp>0 ){
			if(rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+page_counter),page_size,temp_BBT+page_counter*page_size,&rtkn->buf[page_size])){
				rtk_scan_erase_bbt(mtd,(bbt_page+(ppb*i)));
				...
				//mark as bad block;
				if ( isLastPage){
					rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+ppb-1),page_size,temp_BBT,&rtkn->buf[page_size+OOB_BBI_OFF]);
					rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+ppb-2),page_size,temp_BBT,&rtkn->buf[page_size+OOB_BBI_OFF]);
				}else{
					rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)),page_size,temp_BBT,&rtkn->buf[page_size+OOB_BBI_OFF]);
					rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+1),page_size,temp_BBT,&rtkn->buf[page_size+OOB_BBI_OFF]);
				}						
				error_count++;
				break;
			}
...
int rtk_remapBBT_write_fail(struct mtd_info* mtd,unsigned int page,const uint8_t *buf,int oob_required)
{
	...
	for ( i=0; i<backup_offset; i++){
		rtk_scan_read_oob(mtd,rtkn->buf,(block*ppb+i),page_size);
		rtk_scan_write_bbt(mtd,(block_remap*ppb+i),page_size,rtkn->buf,&rtkn->buf[page_size]);
	}

	/* check */
	if(oob_required){
		memcpy(&rtkn->buf[page_size],this->oob_poi,oobsize);
	}

	rtk_scan_write_bbt(mtd,(block_remap*ppb+backup_offset),page_size,(uint8_t*)buf,&rtkn->buf[page_size]);		
	...
	if ( isLastPage ){
		...
		rtk_scan_write_bbt(mtd,(block*ppb+ppb-1),page_size,rtkn->buf,&rtkn->buf[page_size]);
		rtk_scan_write_bbt(mtd,(block*ppb+ppb-2),page_size,rtkn->buf,&rtkn->buf[page_size]);
	}else{
		rtk_scan_erase_bbt(mtd,(block*ppb));
		rtk_scan_write_bbt(mtd,(block*ppb),page_size,rtkn->buf,&rtkn->buf[page_size]);
		rtk_scan_write_bbt(mtd,(block*ppb+1),page_size,rtkn->buf,&rtkn->buf[page_size]);
	}
...
#endif	
			if ( isLastPage){
				rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+(ppb-1)),page_size,rtkn->buf,&rtkn->buf[page_size]);
				rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+(ppb-2)),page_size,rtkn->buf,&rtkn->buf[page_size]);				
			}else{
				rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)),page_size,rtkn->buf,&rtkn->buf[page_size]);
				rtk_scan_write_bbt(mtd,(bbt_page+(ppb*i)+1),page_size,rtkn->buf,&rtkn->buf[page_size]);			
			}
			mem_page_num_tmp = 0; 
			error_count++;
		}
