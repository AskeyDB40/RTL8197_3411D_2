...
#if HCI_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)
...
#ifdef BTCOEX
		rtk_btcoex_parse_event(urb,urb->actual_length);
#endif
#if HCI_VERSION_CODE < KERNEL_VERSION(3, 19, 0)
...
#ifdef BTCOEX
	if (urb->status == 0)		
		rtk_btcoex_parse_l2cap_data_rx(urb,urb->actual_length);
#endif
...
int btusb_send_frame(struct sk_buff *skb)
{
...
#ifdef BTCOEX
		rtk_btcoex_parse_cmd(skb,skb->len);
#endif
	...
	case HCI_ACLDATA_PKT:
		print_acl(skb, 1);
#ifdef BTCOEX
		rtk_btcoex_parse_l2cap_data_tx(skb,skb->len);
#endif
		if (!data->bulk_tx_ep)
			return -ENODEV;
