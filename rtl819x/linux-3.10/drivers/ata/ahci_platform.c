...
*/
	...
	sata_phy_write(portnum, 0x03, 0x0d01);
	sata_phy_write(portnum, 0x04, 0x2000);
	sata_phy_write(portnum, 0x05, 0x01d1);
	sata_phy_write(portnum, 0x06, 0xF438);
	sata_phy_write(portnum, 0x07, 0x245E);

	sata_phy_write(portnum, 0x08, 0x30d9);
	sata_phy_write(portnum, 0x09, 0x538c);
	sata_phy_write(portnum, 0x0a, 0x2008);
	sata_phy_write(portnum, 0x0b, 0x45b9);
	sata_phy_write(portnum, 0x0c, 0x0829);
	sata_phy_write(portnum, 0x0d, 0x1264);
	sata_phy_write(portnum, 0x0e, 0x18c5);
	sata_phy_write(portnum, 0x0f, 0xc00f);
...
int sata_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, ahci_dbg_read, NULL));
}

struct file_operations sata_proc_fops = {
	.open		= sata_single_open,
	.write		= ahci_dbg_write,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
static void ahci_proc_debug_init(void)
{
	...
	proc_create_data("sata", 0, &proc_root,
			 &sata_proc_fops, NULL);
}
#endif
#endif
