...
/*define the GPIO physical address to customer_gpio.h*/
	...
	#elif defined(CONFIG_RTK_VOIP_GPIO_8972D_V100) || defined(CONFIG_RTK_VOIP_GPIO_8881A)
...
*/
	...
	#define RTL_GPIO_DAT_GPIOC0 (1<<16) 
		...
		#define RESET_BTN_PIN           	4
			...
			#define RESET_BTN_PIN           	10  //mark_mfi ,B2 inApplecate 
	#else		
			#define RESET_BTN_PIN           	5
	#endif		
...
int bootbank_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, read_bootbank_proc, NULL));
}

struct file_operations boot_bank_proc_fops = {
        .open           = bootbank_single_open,
	 	.write			= NULL,
        .read           = seq_read,
        .llseek         = seq_lseek,
...
static int write_watchdog_reboot(struct file *file, const char *buffer,
				unsigned long count, void *data);
#if 0
int watchdog_reboot_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, read_proc, NULL));
}
#endif

static ssize_t watchdog_reboot_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_watchdog_reboot(file, userbuf,count, off);
}


struct file_operations watchdog_reboot_proc_fops= {
        .open           = NULL,
	 .write		= watchdog_reboot_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
static int rf_switch_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data);
int rf_switch_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rf_switch_read_proc, NULL));
}

#if 0
static ssize_t rf_switch_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_proc(file, userbuf,count, off);
}
...
struct file_operations rf_switch_proc_fops= {
        .open           = rf_switch_single_open,
	 .write		= NULL,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
...
static int usb_mode_detect_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data);
int usb_mode_detect_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, usb_mode_detect_read_proc, NULL));
}

#if 0
static ssize_t usb_mode_detect_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_proc(file, userbuf,count, off);
}
...
struct file_operations usb_mode_detect_proc_fops = {
        .open           = usb_mode_detect_single_open,
	 .write		= NULL,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
...
int __init rtl_gpio_init(void)
{
	...
	if (res) {
	    ...
	    RTL_W32((PIN_MUX_SEL+0xC), ((RTL_R32(PIN_MUX_SEL+0xC)&(~0xFFFFF))|(0x00022222) ));
	#else
		...
		RTL_W32(PIN_MUX_SEL_2, (RTL_R32(PIN_MUX_SEL_2) | 0x03)); //mark_mfi
...
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("usb_mode_detect", 0, &proc_root,
			 &usb_mode_detect_proc_fops, NULL);
#else
...
#ifdef READ_RF_SWITCH_GPIO
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rf_switch", 0, &proc_root,
			 &rf_switch_proc_fops, NULL);
#else
