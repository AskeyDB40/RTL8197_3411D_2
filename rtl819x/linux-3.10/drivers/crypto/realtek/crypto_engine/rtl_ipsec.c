...
#endif
#if defined(RTK_IPSEC_STATISTIC)
...
do { \
	...
	while( (REG32(LX_CONTROL)&(1<<13)) == 0 ) ; /* wait for Lx1 bus lock okay */ \
...
#if defined(RTK_IPSEC_STATISTIC)
struct cipher_statistic cipher_cnt[RTK_STATTISTIC_MAX];
struct hash_statistic hash_cnt[RTK_STATTISTIC_HASH_MAX];
unsigned ipsec_engine_cnt = 0;

unsigned char *rtk_cipher_mode_to_algname(int mode)
{

	if (mode == 0X00)
		return RTK_ALGNAME_CBC_DES;
	else if (mode == 0x01)
		return RTK_ALGNAME_CBC_3DES;
	else if (mode == 0x02)
		return RTK_ALGNAME_ECB_DES;
	else if (mode == 0x03)
		return RTK_ALGNAME_ECB_3DES;
	else if (mode == 0x20)
		return RTK_ALGNAME_CBC_AES;
	else if (mode == 0x22)
		return RTK_ALGNAME_ECB_AES;
	else if (mode == 0x23)
		return RTK_ALGNAME_CTR_AES;
	else
		return NULL;
}

unsigned char *rtk_hash_mode_to_algname(int mode)
{

	if (mode == 0x00)
		return RTK_ALGNAME_MD5;
	else if (mode == 0x01)
		return RTK_ALGNAME_SHA1;
	else
		return NULL;
}

void rtk_ipsec_update_cipher_count(int flag, int mode, struct cipher_statistic *cipher_cnt, int length)
{
	int i = 0;
	int alg_name_len = 0;
	...
	alg_name = rtk_cipher_mode_to_algname(mode);
	if (!alg_name)
		return ;
	
	alg_name_len = strlen(alg_name);
	if (alg_name_len > RTK_STATTISTIC_ALGNAME_LEN_MAX){
		alg_name_len = RTK_STATTISTIC_ALGNAME_LEN_MAX;
	}
	//exist?
	for (i = 0; i < length; i++){
		if (cipher_cnt[i].valid && !memcmp(alg_name, cipher_cnt[i].alg_name, alg_name_len)){
			//exist? update and return
			if (flag == RTK_STATTISTIC_CIPHER_INIT){
				cipher_cnt[i].cipher_init_cnt++;
			}
			else if (flag == RTK_STATTISTIC_CIPHER_SETKEY){
				cipher_cnt[i].cipher_setkey_cnt++;

			}
			else if (flag == RTK_STATTISTIC_CIPHER){
				cipher_cnt[i].cipher_cnt++;
			}
			return;
		}
	}
	...
	if (i == length){
		printk("%s %d statistic entry full!!\n", __func__, __LINE__);
		return;
	}

	memcpy(cipher_cnt[i].alg_name, alg_name, alg_name_len);
	cipher_cnt[i].valid = 1;
	if (flag == RTK_STATTISTIC_CIPHER_INIT){
		cipher_cnt[i].cipher_init_cnt++;
	}
	else if (flag == RTK_STATTISTIC_CIPHER_SETKEY){
...
void rtk_ipsec_update_hash_count(int flag, int mode, struct hash_statistic *hash_cnt, int length)
{
	int i = 0;
	int alg_name_len = 0;
	unsigned char *alg_name = NULL;
	...
	alg_name = rtk_hash_mode_to_algname(mode);
	if (!alg_name)
		return ;
	...
	//exist?
	for (i = 0; i < length; i++){
		if (hash_cnt[i].valid && !memcmp(alg_name, hash_cnt[i].hash_name, alg_name_len)){
			//exist? update and return
			if (flag == RTK_STATTISTIC_HASH_INIT){
				hash_cnt[i].hash_init_cnt++;
			}
			else if (flag == RTK_STATTISTIC_HASH_UPDATE){
				hash_cnt[i].hash_update_cnt++;

			}
			else if (flag == RTK_STATTISTIC_HASH_FINAL){
				hash_cnt[i].hash_final_cnt++;
			}
			else if (flag == RTK_STATTISTIC_HASH_DIGEST){
				hash_cnt[i].hash_digest_cnt++;
			}
			return;
		}
	}
	...
	memcpy(hash_cnt[i].hash_name, alg_name, alg_name_len);
	hash_cnt[i].valid = 1;
	if (flag == RTK_STATTISTIC_HASH_INIT){
		hash_cnt[i].hash_init_cnt++;
	}
...
static int rtk_ipsec_statistic_read(struct seq_file *s, void *v)
{
	int i = 0;

	seq_printf(s, "ipsec_engine_cnt=%u \n", ipsec_engine_cnt);

	#if defined(CONFIG_REALTEK_CRYPTO_API)
	seq_printf(s, "\ndump crypto api statistic:\n");	
	seq_printf(s, "crypto_api_recv_cnt=%u crypto_api_send_cnt=%u crypto_api_only_enc_cnt=%u crypto_api_only_dec_cnt=%u \n", crypto_api_recv_cnt, crypto_api_send_cnt, crypto_api_only_enc_cnt, crypto_api_only_dec_cnt);
	#endif
	...
	seq_printf(s, "\ndump cipher statistic:\n");
	for (i = 0; i < RTK_STATTISTIC_MAX; i++){
		//if (cipher_cnt[i].valid)
		{
			seq_printf(s, "name: %s \n", cipher_cnt[i].alg_name);
			seq_printf(s, "init count=%u setkey count=%u cipher count=%u valid=%u\n\n", cipher_cnt[i].cipher_init_cnt, cipher_cnt[i].cipher_setkey_cnt, cipher_cnt[i].cipher_cnt, cipher_cnt[i].valid);
		}
	}
	
	seq_printf(s, "\ndump hash statistic:\n");
	for (i = 0; i < RTK_STATTISTIC_HASH_MAX; i++){
		//if (hash_cnt[i].valid)
		{
			seq_printf(s, "name: %s \n", hash_cnt[i].hash_name);
			seq_printf(s, "init count=%u update count=%u final count=%u digest=%u valid=%u\n\n", hash_cnt[i].hash_init_cnt,hash_cnt[i].hash_update_cnt, hash_cnt[i].hash_final_cnt, hash_cnt[i].hash_digest_cnt, hash_cnt[i].valid);
		}
	}
...
#else
static int32 rtk_ipsec_statistic_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	int len = 0;
	int i = 0;

	len += sprintf(page, "ipsec_engine_cnt=%u \n", ipsec_engine_cnt);

	#if defined(CONFIG_REALTEK_CRYPTO_API)
	len += sprintf(page, "\ndump crypto api statistic:\n");	
	len += sprintf(page, "crypto_api_recv_cnt=%u crypto_api_send_cnt=%u crypto_api_only_enc_cnt=%u crypto_api_only_dec_cnt=%u \n", crypto_api_recv_cnt, crypto_api_send_cnt, crypto_api_only_enc_cnt, crypto_api_only_dec_cnt);
	#endif
	...
	for (i = 0; i < RTK_STATTISTIC_MAX; i++){
		//if (cipher_cnt[i].valid)
		{
			...
			len += sprintf(page, "init count=%u setkey count=%u cipher count=%u valid=%u\n\n", cipher_cnt[i].cipher_init_cnt, cipher_cnt[i].cipher_setkey_cnt, cipher_cnt[i].cipher_cnt, cipher_cnt[i].valid);
		}
	}
	...
	for (i = 0; i < RTK_STATTISTIC_HASH_MAX; i++){
		...
		{
			len += sprintf(page, "name: %s \n", hash_cnt[i].hash_name);
			len += sprintf(page, "init count=%u update count=%u final count=%u digest=%u valid=%u\n\n", hash_cnt[i].hash_init_cnt,hash_cnt[i].hash_update_cnt, hash_cnt[i].hash_final_cnt, hash_cnt[i].hash_digest_cnt, hash_cnt[i].valid);
		}
	}
...
#endif
static int32 rtk_ipsec_statistic_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	{
		tmpbuf[len-1] = '\0';
		if(!memcmp(tmpbuf, "clear", strlen("clear")))
		{
			//clear all
			memset((void *)&cipher_cnt, '\0', sizeof(cipher_cnt));
			memset((void *)&hash_cnt, '\0', sizeof(hash_cnt));
			ipsec_engine_cnt = 0;
			#if defined(CONFIG_REALTEK_CRYPTO_API)
...
int rtk_ipsec_statistic_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtk_ipsec_statistic_read,NULL));
}
static ssize_t rtk_ipsec_statistic_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtk_ipsec_statistic_write(file, userbuf,count, off);
}
struct file_operations rtk_ipsec_statistic_proc_fops= {
        .open           = rtk_ipsec_statistic_single_open,
        .write		    = rtk_ipsec_statistic_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
...
int32 init_ipsec_proc(void)
{
	#ifndef CONFIG_RTL_PROC_NEW
	struct proc_dir_entry *ipsec_statistic_entry;
	#endif

	#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtk_ipsec_status",0,&proc_root,&rtk_ipsec_statistic_proc_fops,NULL);
	#else
	ipsec_statistic_entry=create_proc_entry("rtk_ipsec_status",0,NULL);
	if (ipsec_statistic_entry)
	{
		ipsec_statistic_entry->read_proc=rtk_ipsec_statistic_read;
		ipsec_statistic_entry->write_proc=rtk_ipsec_statistic_write;
	}
	#endif
