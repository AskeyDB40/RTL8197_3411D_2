...
/*------statistic-----------*/
#define RTK_IPSEC_STATISTIC
#if defined(RTK_IPSEC_STATISTIC)
#define RTK_STATTISTIC_MAX 10
#define RTK_STATTISTIC_ALGNAME_LEN_MAX 16

#define RTK_STATTISTIC_CIPHER_INIT 1
#define RTK_STATTISTIC_CIPHER_SETKEY 2
#define RTK_STATTISTIC_CIPHER 3

#define RTK_ALGNAME_CBC_DES "cbc(des)"
#define RTK_ALGNAME_CBC_3DES "cbc(des3_ede)"
#define RTK_ALGNAME_ECB_DES "ecb(des)"
#define RTK_ALGNAME_ECB_3DES "ecb(des3_ede)"
#define RTK_ALGNAME_CBC_AES "cbc(aes)"
#define RTK_ALGNAME_ECB_AES "ecb(aes)"
#define RTK_ALGNAME_CTR_AES "ctr(aes)"


#define RTK_STATTISTIC_HASH_MAX 5

#define RTK_STATTISTIC_HASH_INIT   1
#define RTK_STATTISTIC_HASH_UPDATE 2
#define RTK_STATTISTIC_HASH_FINAL  3
#define RTK_STATTISTIC_HASH_DIGEST 4

#define RTK_ALGNAME_MD5 "md5"
#define RTK_ALGNAME_SHA1 "sha1"


struct cipher_statistic {
	unsigned char alg_name[RTK_STATTISTIC_ALGNAME_LEN_MAX];
	unsigned int cipher_init_cnt;
	unsigned int cipher_setkey_cnt;
	unsigned int cipher_cnt;
	unsigned int valid;
};

struct hash_statistic {
	unsigned char hash_name[RTK_STATTISTIC_ALGNAME_LEN_MAX];
	unsigned int hash_init_cnt;
	unsigned int hash_update_cnt;
	unsigned int hash_final_cnt;
	unsigned int hash_digest_cnt;
	unsigned int valid;
};

extern struct cipher_statistic cipher_cnt[RTK_STATTISTIC_MAX];
extern struct hash_statistic hash_cnt[RTK_STATTISTIC_HASH_MAX];
extern unsigned ipsec_engine_cnt;

#if defined(CONFIG_REALTEK_CRYPTO_API)
extern unsigned int crypto_api_recv_cnt;
extern unsigned int crypto_api_send_cnt;
extern unsigned int crypto_api_only_enc_cnt;
extern unsigned int crypto_api_only_dec_cnt;
#endif

extern void rtk_ipsec_update_cipher_count(int flag, int mode, struct cipher_statistic *cipher_cnt, int length);
extern int32 init_ipsec_proc(void);
extern void rtk_ipsec_update_hash_count(int flag, int mode, struct hash_statistic *hash_cnt, int length);
