			    ...
			    u8 addr_type, u8 val[16], bdaddr_t *rpa)
{
	...
	if (!irk) {
		...
		BT_INFO("%s: adding id %u, %02X:%02X:%02X:%02X:%02X:%02X",
			__func__, addr_type, bdaddr->b[5], bdaddr->b[4],
			bdaddr->b[3], bdaddr->b[2], bdaddr->b[1],
			bdaddr->b[0]);

		BT_INFO("%s: adding rpa %02X:%02X:%02X:%02X:%02X:%02X",
			__func__, rpa->b[5], rpa->b[4],
			rpa->b[3], rpa->b[2], rpa->b[1],
			rpa->b[0]);
...
//	bacpy(&irk->rpa, rpa);
	...
	if (bacmp(rpa, BDADDR_ANY)) {
		BT_INFO("%s: update rpa bdaddr", __func__);
		bacpy(&irk->rpa, rpa);
	}

	BT_INFO("%s: id %u, %02X:%02X:%02X:%02X:%02X:%02X",
		__func__, addr_type, bdaddr->b[5], bdaddr->b[4],
		bdaddr->b[3], bdaddr->b[2], bdaddr->b[1],
		bdaddr->b[0]);

	BT_INFO("%s: rpa %02X:%02X:%02X:%02X:%02X:%02X",
		__func__, rpa->b[5], rpa->b[4],
		rpa->b[3], rpa->b[2], rpa->b[1],
		rpa->b[0]);
	return irk;
