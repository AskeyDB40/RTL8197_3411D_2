				     ...
				     u16 dlen, void *data)
{
	...
	struct l2cap_hdr *lh;
	int len;
	int hlen = L2CAP_HDR_SIZE;
	int count;
	int sent = 0;

	len = hlen + (1 + dlen);
	count = min_t(unsigned int, (conn->mtu - hlen - 1), dlen);

	BT_DBG("code 0x%2.2x, dlen 0x%4.4x, count 0x%4.4x, mtu 0x%4.4x.",
	       code, dlen, count, conn->mtu);

	skb = bt_skb_alloc(hlen + 1 + count, GFP_ATOMIC);
	if (!skb) {
		BT_ERR("Failed to alloc first l2cap pkt for smp cmd.");
		return NULL;
	}
	skb->priority = HCI_PRIO_MAX;

	lh = (struct l2cap_hdr *)skb_put(skb, hlen);
	lh->len = cpu_to_le16(dlen + 1);
	...
	BT_DBG("sent 0x%4.4x, dlen 0x%4.4x.", sent, dlen);

	/* Continuation fragments (no L2CAP header) */
	frag = &skb_shinfo(skb)->frag_list;
	while (dlen) {
		count = min_t(unsigned int, conn->mtu, dlen);

		*frag = bt_skb_alloc(count, GFP_ATOMIC);
		if (!*frag) {
			BT_ERR("Failed to alloc fragment for smp.");
			return NULL;
		}
...
static void smp_send_cmd(struct l2cap_conn *conn, u8 code, u16 len, void *data)
{
	...
	if (!skb) {
		BT_ERR("Failed to smp build cmd.");
		return;
	}
...
int smp_conn_security(struct hci_conn *hcon, __u8 sec_level)
{
	...
	BT_INFO("%s: conn %p hcon %p level 0x%2.2x", __func__, conn, hcon,
		sec_level);
	...
	BT_INFO("%s: check conn->smp", __func__);
	chan = conn->smp;
	if (!chan) {
				   ...
				   struct sk_buff *skb)
{
	...
	BT_INFO("%s: dst, type %u, bdaddr %02X:%02X:%02X:%02X:%02X:%02X",
		__func__, hcon->dst_type, hcon->dst.b[5],
		hcon->dst.b[4], hcon->dst.b[3], hcon->dst.b[2],
		hcon->dst.b[1], hcon->dst.b[0]);
	...
	BT_INFO("id, addr %02X:%02X:%02X:%02X:%02X:%02X, type %u",
		smp->id_addr.b[5], smp->id_addr.b[4], smp->id_addr.b[3],
		smp->id_addr.b[2], smp->id_addr.b[1], smp->id_addr.b[0],
		smp->id_addr_type);
...
static void smp_ready_cb(struct l2cap_chan *chan)
{
	...
	BT_INFO("%s: chan %p", __func__, chan);
